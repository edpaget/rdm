...
# rdm-wf-plan-review.js / lib/plan-review.mjs row. That engine dispatches finder
# and refuter agents only now, so it has no mechanical site to trim. The rows
# that remain belong to the three engines commit 4 has yet to reach.
MECHANICAL_SITES=$(
    cat <<'SITES'
...
# 3, not 4: plan-review's `model:mechanical` bootstrap was deleted with the rest
# of its mechanical lane (no-mechanical-agents-in-workflows phase 34, commit 3).
MECH_BOOTSTRAPS=3 # document/backlog/estimate, one `model:mechanical` each
