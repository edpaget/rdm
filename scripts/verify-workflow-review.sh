...
// DELETED (no-mechanical-agents-in-workflows phase 34, commit 2): sub-block (9),
// the plan-lane anchor-degradation accounting battery. `persistAccounting` /
// `degradationSummaryClause` existed only to read a persisting AGENT's
// self-report; the plan driver now RETURNS the ladder as command text and runs
// nothing, so there is no ack to account for. Deleted and named.
