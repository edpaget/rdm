...
#   positioned BEFORE phase design, in `skill-roadmap-cli.md`.
#
#   PREDATES  — the same capture offered from interactive `rdm-plan-review`
#   for a roadmap/task that predates the artifact, in
#   `skill-plan-review-cli.md`, positioned strictly OUTSIDE the
...
#      claude --skills` emission, re-running the same marker/position/bound
#      checks on the emitted tree.
...
for f in "$TPL_ROADMAP_CLI" "$TPL_PLAN_REVIEW_CLI"; do
...
grep -qF "$AUTHORING_MARKER" "$TPL_ROADMAP_CLI" || fail "$TPL_ROADMAP_CLI: missing authoring interview step marker '$AUTHORING_MARKER'"
pass "authoring interview step marker present in skill-roadmap-cli.md"

grep -qF "$PREDATES_MARKER" "$TPL_PLAN_REVIEW_CLI" || fail "$TPL_PLAN_REVIEW_CLI: missing predates-the-artifact capture step marker '$PREDATES_MARKER'"
pass "predates-the-artifact capture step marker present in skill-plan-review-cli.md"

say "1a. Step position: interview runs BEFORE phase design"
interview_line=$(grep -n -F "$AUTHORING_MARKER" "$TPL_ROADMAP_CLI" | head -1 | cut -d: -f1)
design_line=$(grep -n -F "$DESIGN_MARKER" "$TPL_ROADMAP_CLI" | head -1 | cut -d: -f1)
[ -n "$interview_line" ] || fail "$TPL_ROADMAP_CLI: interview marker line not found"
[ -n "$design_line" ] || fail "$TPL_ROADMAP_CLI: '$DESIGN_MARKER' line not found"
[ "$interview_line" -lt "$design_line" ] ||
    fail "$TPL_ROADMAP_CLI: interview step (line $interview_line) must precede '$DESIGN_MARKER' (line $design_line)"
pass "interview step precedes phase design in skill-roadmap-cli.md"

say "1b. Marker region: predates-capture lands OUTSIDE (before) rdm:review-spec"
predates_line=$(grep -n -F "$PREDATES_MARKER" "$TPL_PLAN_REVIEW_CLI" | head -1 | cut -d: -f1)
begin_line=$(grep -n -F "$REVIEW_SPEC_BEGIN" "$TPL_PLAN_REVIEW_CLI" | head -1 | cut -d: -f1)
[ -n "$predates_line" ] || fail "$TPL_PLAN_REVIEW_CLI: predates-capture marker line not found"
[ -n "$begin_line" ] || fail "$TPL_PLAN_REVIEW_CLI: '$REVIEW_SPEC_BEGIN' marker line not found"
[ "$predates_line" -lt "$begin_line" ] ||
    fail "$TPL_PLAN_REVIEW_CLI: predates-capture step (line $predates_line) must land before the generated review-spec block (line $begin_line)"
pass "predates-capture step lands strictly outside the rdm:review-spec marker region"
...
awk '/^---$/{n++; next} n==1' "$TPL_ROADMAP_CLI" | grep -qF 'AskUserQuestion' ||
    fail "$TPL_ROADMAP_CLI: allowed-tools frontmatter must grant AskUserQuestion"
pass "AskUserQuestion present in rdm-roadmap allowed-tools"
...
fallback_line=$(grep -n -F "$ROADMAP_FALLBACK_MARKER" "$TPL_ROADMAP_CLI" | head -1 | cut -d: -f1)
[ -n "$fallback_line" ] || fail "$TPL_ROADMAP_CLI: missing roadmap-already-exists fallback marker"
next_line=$(awk -v start="$fallback_line" 'NR>start && /^5\. \*\*/{print NR; exit}' "$TPL_ROADMAP_CLI")
[ -n "$next_line" ] || next_line=$(wc -l <"$TPL_ROADMAP_CLI")
block=$(sed -n "${fallback_line},${next_line}p" "$TPL_ROADMAP_CLI")
echo "$block" | grep -qF '## Intent' || fail "$TPL_ROADMAP_CLI: roadmap-exists fallback block missing '## Intent' reference"
echo "$block" | grep -qF 'rdm roadmap update' || fail "$TPL_ROADMAP_CLI: roadmap-exists fallback missing 'rdm roadmap update' write-back call"
pass "roadmap-exists fallback block (step 4) carries '## Intent' and its write-back call"
...
pass "intent-capture step (step 6) carries '## Intent' and its write-back calls in skill-plan-review-cli.md"
...
pass "canonical grammar labels and captured-vs-optional sentence present in both extracted step blocks"
...
pass "no rdm-intent command-surface literal in either template"
...
say "3. Dynamic re-check against 'rdm agent-config claude --skills'"
...
for f in "$EMIT_ROADMAP_CLI" "$EMIT_PLAN_REVIEW_CLI"; do
    [ -f "$f" ] || fail "expected emitted skill file not found: $f"
done

interview_line=$(grep -n -F "$AUTHORING_MARKER" "$EMIT_ROADMAP_CLI" | head -1 | cut -d: -f1)
design_line=$(grep -n -F "$DESIGN_MARKER" "$EMIT_ROADMAP_CLI" | head -1 | cut -d: -f1)
[ -n "$interview_line" ] || fail "emitted $EMIT_ROADMAP_CLI: interview marker missing"
[ -n "$design_line" ] || fail "emitted $EMIT_ROADMAP_CLI: '$DESIGN_MARKER' missing"
[ "$interview_line" -lt "$design_line" ] ||
    fail "emitted $EMIT_ROADMAP_CLI: interview step must precede phase design"
pass "emitted skill-roadmap SKILL.md carries the interview step before phase design"

predates_line=$(grep -n -F "$PREDATES_MARKER" "$EMIT_PLAN_REVIEW_CLI" | head -1 | cut -d: -f1)
begin_line=$(grep -n -F "$REVIEW_SPEC_BEGIN" "$EMIT_PLAN_REVIEW_CLI" | head -1 | cut -d: -f1)
[ -n "$predates_line" ] || fail "emitted $EMIT_PLAN_REVIEW_CLI: predates-capture marker missing"
[ -n "$begin_line" ] || fail "emitted $EMIT_PLAN_REVIEW_CLI: '$REVIEW_SPEC_BEGIN' marker missing"
[ "$predates_line" -lt "$begin_line" ] ||
    fail "emitted $EMIT_PLAN_REVIEW_CLI: predates-capture step must land outside the generated review-spec block"
pass "emitted skill-plan-review SKILL.md carries the predates-capture step outside rdm:review-spec"
...
pass "emitted tree carries all five bounds, no forbidden literals, in both SKILL.md files"
...
pass "emitted tree carries the canonical grammar labels and captured-vs-optional sentence in both SKILL.md files"
