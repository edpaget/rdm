...
Each of these generator scripts (`gen-workflow-review.sh`, `gen-workflow-backlog.sh`, `gen-workflow-document.sh`, and `gen-workflow-estimate.sh` above) also whole-file-syncs its consumer's embedded `rdm-core/src/templates/workflows/` copy in the same run (shared logic lives in `scripts/lib/gen-workflow-block.sh`), so that copy needs no separate regeneration step either.

When you hand-edit a stamped block (lines between the `// >>> … <<<` markers) or an embedded copy, the corresponding drift gate turns red. Regenerate with the canonical generator script (e.g., `scripts/gen-workflow-review.sh`, no `--check`), then the gate passes again.
