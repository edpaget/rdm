...
**Generated with hand-maintained code outside the markers:** Some workflow files contain one or more large stamped blocks (generated by a script) plus surrounding hand-maintained code (comments, the `meta` export, thin runtime-entry wiring). The stamped portions must never be hand-edited; the surrounding code is edited freely:
- `.claude/workflows/rdm-wf-review-refute-fix.js` — lines 52–1778 are generated by `scripts/gen-workflow-review.sh` (stamped from `.claude/workflows/lib/review.mjs`); the trailing driver code is executed by `rdm-cli/tests/workflow_review/engine.rs`, not byte-checked against a single lib module
- `.claude/workflows/rdm-wf-plan-review.js` — two stamped blocks, both generated by `scripts/gen-workflow-review.sh`: the review-refute-fix block from `.claude/workflows/lib/review.mjs`, and the `plan-review-driver` block from `.claude/workflows/lib/plan-review.mjs` (verified byte-identical by `rdm-core/tests/workflow_plan_review_driver.rs`); the surrounding runtime-entry wiring stays hand-maintained
- `.claude/workflows/rdm-wf-backlog.js` — stamped block generated by `scripts/gen-workflow-backlog.sh` from `.claude/workflows/lib/backlog.mjs` (its `--check`, including the embedded and plugin copies, is run by `rdm-cli/tests/workflow_passes/backlog.rs`'s `generator_*` tests); the surrounding code stays hand-maintained
- `.claude/workflows/rdm-wf-document.js` — stamped block generated by `scripts/gen-workflow-document.sh` from `.claude/workflows/lib/document.mjs` (its `--check`, including the embedded and plugin copies, is run by `rdm-cli/tests/workflow_passes/document.rs`'s `generator_*` tests); the surrounding code stays hand-maintained

Each of these generator scripts (`gen-workflow-review.sh`, `gen-workflow-backlog.sh`, `gen-workflow-document.sh`, and `gen-workflow-estimate.sh` above) also whole-file-syncs its consumer's embedded `rdm-core/src/templates/workflows/` copy AND its checked-in `plugins/rdm/workflows/<engine>.js` copy, both in the same run (shared logic lives in `scripts/lib/gen-workflow-block.sh`), so neither copy needs a separate regeneration step. The rest of `plugins/rdm/` — the 11 skills' markdown and the manifest — is untouched by these generators and still needs the "Canonical Regeneration Command" above.

When you hand-edit a stamped block (lines between the `// >>> … <<<` markers) or an embedded/plugin copy, the corresponding drift gate turns red. Regenerate with the canonical generator script (e.g., `scripts/gen-workflow-review.sh`, no `--check`), then the gate passes again.

When you hand-edit `rdm-wf-review-refute-fix.js`'s trailing driver code in a way that breaks its behaviour (which should not happen), the `engine::` cases of `cargo nextest run -p rdm-cli --test workflow_review` turn red. Restore the file from git.
