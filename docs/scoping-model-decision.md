...
**Why this is still an open question:** This is precisely the mechanism that phase 4's constraints identify: "The derived indexes land in every session's journal because `ops::mutate` regenerates them on every mutation. Journaling them is correct; reconciling them at commit time is phase 5's problem." Solving it requires a journal-scoped Store view or similar mechanism so that `generate_index_for_project` (and its `build_project_index` subroutines) read only from HEAD + the caller's journaled paths, not from the live filesystem. This is not a minor tweak — it fundamentally changes how the index-generation functions see the repository state.

**Whose responsibility it is (amended after phase 4 shipped).** An earlier draft of this section assigned the index-scoping work to phase 4. That contradicted phase 4's own constraint quoted immediately above, and the constraint is the one that holds: phase 4 changes no commit behavior at all, so it cannot be the phase that scopes what a commit contains. The split is:

- **Phase 4 (done):** journal the derived indexes. They land in *every* session's journal, which is correct — every session really did rewrite them. See [`docs/session-identity.md`](session-identity.md) § Exactness, which asserts this deliberately.
- **Phase 5:** scope the index-generation functions to respect the changeset boundary — modify `build_project_index`, or introduce a wrapper providing a constrained Store view (a virtual overlay or journal-backed store) to the `list_roadmaps`/`list_phases`/`list_tasks` calls — so that when a session commits, the `INDEX.md` in the tree reflects only entities that exist in that tree.
