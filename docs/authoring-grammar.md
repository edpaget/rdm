...
The code-mode `ac` review dimension (`.claude/workflows/lib/review.mjs`) honors this pattern only when it is followed exactly: a deferral declared *outside* the criterion — in the Approach section or another follow-up note — with the criterion's own wording already excluding the deferred part, is treated as a scope decision that bounds the criterion rather than as an unmet gap. A caveat or exception written *inside* the criterion text itself, as in the problematic example below, is still graded unmet, whether or not the same deferral is also named elsewhere in the body. A deferral or caveat that first appears in the implementation diff or its commentary — with no antecedent in the body at all — still blocks as well.
