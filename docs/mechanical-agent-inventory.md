...
| `fetch:<kind>` (`fetch:task` / `fetch:phase`) | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | local shim **and the in-repo dispatch orchestrator** | **hoisted in fact — PRIORITY** | Same — see below. Since `agent-orchestrated-dispatch` phase 23 the in-repo `rdm-dispatch-phase` orchestrator passes the item's body and tags as `fetched`, out of the same `show --format json` read it already makes for the difficulty tier. Keeps its fail-CLOSED empty-body behavior (`built.fetchFailed`) on both paths. | partly (distributed-caller path) |
