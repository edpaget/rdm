...
**34 labelled `agent()` call sites** across the five workflow scripts (moved 33 → 34: code review
`2026-09-21-1218-d618` added `fetch:plan-body-check` to `rdm-wf-plan-review.js`, closing finding
`correctness-fetch-plan-no-content-integrity-check` — see that row's own history below the
per-file table further down):
...
`fetch:roadmap` / `fetch:roadmap-body-check` / `fetch:<kind>` / `fetch:plan` /
`fetch:plan-body-check` / `fetch:wontfix` / `act:round-note:*` / `gate:clear-tag:*`, which sit
inside the byte-copied `plan-review-driver` block. No mechanical site sits inside a
...
| `verify:run` | `rdm-wf-dispatch-phase.js` | unprojected driver | yes | n/a — the command is resolved at Stage 0, but the RUN itself cannot be hoisted | **irreducible** | The phase-time verification gate (see [`verify-gate.md`](verify-gate.md)): it executes the project's single declared command in the item's worktree once per implementation attempt, plus once more when the Act step changed code (`verify-gate.md` § 8), so its input is state the run itself just produced. Nothing a caller could pre-compute substitutes for it. The command STRING is hoistable and rides along on the Stage-0 fetch (`verify` on `PHASE_META`/`TASK_META`), which is why this row adds one call site and no new fetch. | no |
...
| `model:mechanical` | `rdm-wf-plan-review.js` | unprojected runtime entry | no | local shim (`rdm-plan-review`) **and the in-repo dispatch orchestrator** | **hoisted in fact** | Same bootstrap read — extended by `thread-plan-review-judgment-models` to also resolve `review-find`/`review-verify` in this one call, threaded through as `findModel`/`verifyModel`. Since `agent-orchestrated-dispatch` phase 23 the in-repo `rdm-dispatch-phase` orchestrator resolves all three ids **untiered** in its identity-pin step and passes them, so this agent no longer fires on the dispatch path. All-or-nothing (`hoistedModelsComplete`): two of the three still spawns the bootstrap. | partly (distributed-caller path) |
...
| `fetch:roadmap-intent` | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` (gate: `verify-workflow-review.sh` §5b-drift) | no | local shim **and the in-repo dispatch orchestrator** | **hoisted in fact** (was classed *irreducible on its own path* until a caller-suppliable argument existed) | Added by the `review-gate-intent` roadmap's plan-review intent gate. Reads the PARENT ROADMAP's body so a standalone `--phase` target can inherit its recorded `## Intent` — the one target shape that has no roadmap body in hand. Deliberately NOT added to the `--roadmap` fan-out (that path reuses the body `fetch:roadmap` already transcribed, preserving the exactly-ONE-mechanical-agent-per-roadmap-target rule above `buildRoadmapFetchPrompt`), and never fired for a `--task` or `--implementation-plan` target. **A hoisted `fetched` does NOT suppress it** — that payload is the phase's OWN body and tags, a different document from the parent roadmap's body this reads — a claim this row asserted incorrectly until `agent-orchestrated-dispatch` phase 23. That phase added its own hoist instead: an optional top-level `roadmapBody` (the parent roadmap's body verbatim), shape-guarded by `hoistedRoadmapBodyOk` in the manner of `hoistedFetchedOk` and **deliberately independent of it** — two documents, two agents, no combined completeness guard, so supplying either alone is legal. The engine runs its own `extractIntent` over the raw body, so there is exactly one implementation of the extraction on both paths. Any failure — thrown agent, empty transcript, unparseable JSON, a hoisted body with no `## Intent`, a throw out of `extractIntent` — degrades to `{ hasIntent: false, intent: null }` rather than an error, because the dimension it feeds is non-blocking by construction; the hoist branch sits INSIDE the existing `try` so that guarantee holds by construction rather than by a second copy of the recovery. | partly (distributed-caller path) |
| `fetch:<kind>` (`fetch:task` / `fetch:phase`) | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | local shim only | **hoistable — PRIORITY** | Same — see below. `agent-orchestrated-dispatch` phase 23 had the in-repo `rdm-dispatch-phase` orchestrator pass the item's body and tags as `fetched`; phase 30 removed that, because the dispatch no longer reviews the ITEM document at all — it dispatches `kind: 'implementation-plan'` over the plan body, a branch that returns before the fetch block, so this agent is not reachable on the dispatch path and there is nothing left there to hoist. The `fetched` hoist itself is unchanged and still suppresses this agent for the local shim and any other direct caller, keeping its fail-CLOSED empty-body behavior (`built.fetchFailed`) on both paths. | partly (distributed-caller path) |
| `fetch:plan` | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | the in-repo dispatch orchestrator only — the `implementation-plan` branch this site lives in is reached solely by `rdm-dispatch-phase`'s step 6; the local `rdm-plan-review` shim never targets an implementation plan | **not a new hoist candidate** | Added by `agent-orchestrated-dispatch` phase 32 to close the defect `fetch:<kind>`'s own row above records phase 30 causing: with the item-document read gone, the orchestrator became the ONLY thing that could supply the plan body, and had to reproduce it verbatim in its own `Workflow(...)` call arguments (19,717 characters on the first real dispatch that hit it). `planSlug` and `planText` are now MUTUALLY EXCLUSIVE (revised by code review `2026-09-21-1218-d618`, finding `ac-4-dual-supply-divergence` — supplying both used to grade `planText` verbatim while persisting against `plan/<planSlug>`, a document that fetch never read; see decision 3 of the plan `rdm:plan/engine-resolves-plan-by-slug`), so a present `planSlug` always fires this read and there is nothing further to hoist. One bounded retry, `RAW_STDOUT_SCHEMA` verbatim-transcription contract, `extractPlanFromJson` rejects unless the fetched `json.slug` equals `planSlug` — the same slug the persist ref (`'plan/' + planSlug`) is built from — and fails CLOSED (`escalated`, `fetchError: true`, nothing persisted) on a second untrustworthy or mismatched payload, rather than grading a body that might not be the recorded plan. | partly (distributed-caller path) |
| `fetch:plan-body-check` | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | the in-repo dispatch orchestrator only, same reachability as `fetch:plan` above | **not a hoist candidate** | Added by code review `2026-09-21-1218-d618`, finding `correctness-fetch-plan-no-content-integrity-check`: `extractPlanFromJson` on the row above is identity- and non-emptiness-only, so a schema-valid, identity-correct `fetch:plan` transcript carrying a fabricated one-line body (e.g. "Fetched the plan successfully.") passed cleanly and would have persisted an unearned `approve` verdict — the exact failure class `fetch:roadmap-body-check` above already exists to close for the roadmap-body unit. This is that same treatment applied to `fetch:plan`: re-reads `plan show <slug>` independently and reports only a checkable length/first-line property, compared against `fetch:plan`'s transcribed body via `roadmapBodyVerified` (reused verbatim — that comparison function is fully generic and has no roadmap-specific logic despite its name). Runs only once a candidate body has cleared `extractPlanFromJson`; a confirmed disagreement discards the fetch and routes through the branch's existing fail-closed return, an unavailable/erroring check proceeds unverified. Costs one extra mechanical call per slug-resolved implementation-plan dispatch — the same documented tension `fetch:roadmap-body-check` already accepted. | partly (distributed-caller path) |
| `fetch:wontfix` | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | local shim **and the in-repo dispatch orchestrator** | **hoisted in fact** | One `rdm search` covering the whole run, read-only, before any unit is reviewed. `args.wontFixedTexts` has existed since `workflow-token-reduction` phase 3; `agent-orchestrated-dispatch` phase 23 decided the caller side and the orchestrator now supplies it. **From `rdm task list --tag plan-review --status wont-fix --format json`, passing TITLES, not from `rdm search`:** `search` truncates at its default `--limit 20` where the real corpus is larger, and its JSON carries no `body` field at all — so the fallback prompt's "title and body" instruction forces a `task show` per result. Title-only is the strictly UNDER-suppressing direction, which is the safe one for `wontFixOverlapMatches`' ≥70%-overlap heuristic (a false suppress removes a live finding; a false miss only re-reports something already dismissed). An empty array is a legal, meaningful value and is not the same as omitting the key. The fallback prompt itself is left byte-unchanged — its `--limit` truncation and phantom `body` field are filed as `task/plan-review-wontfix-search-truncates-at-twenty`. | partly (distributed-caller path) |
...
| `rdm-wf-plan-review.js` | `fetch:roadmap`, `fetch:roadmap-body-check`, `fetch:roadmap-intent`, `fetch:<kind>`, `fetch:plan`, `fetch:plan-body-check`, `fetch:wontfix`, `gate:clear-tag:<kind>:<ident>` | **byte-copied** — inside the `plan-review-driver` block; edit `lib/plan-review.mjs` first, then copy verbatim. `verify-workflow-review.sh` §5b-drift gates the pair |
| `rdm-wf-plan-review.js` | `model:mechanical` | unprojected driver (below `plan-review-driver:end`) — edit in place |

19 sites, three maintenance routes, all threaded. This total is corrected here, not merely
incremented: it read "17" immediately before this edit, which already undercounted the table above
by one (4+2+4+7+1 = 18, not 17 — code review `2026-09-21-1218-d618` finding
`arch-inventory-site-total-stale`); this edit's own `fetch:plan-body-check` addition (see the
per-file table row above) moves the true count from that corrected 18 to 19. (The "review-persistence
phase" paragraph further below independently claims this same total already reached 19 earlier, from
a different pair of additions — that paragraph was not reconciled against the intervening
`rdm-wf-dispatch-phase.js` retirement or against this correction, and is left as a dated record
rather than a live-checked figure, matching this document's stated practice for other historical
deltas.) **Off-limits regardless:**
