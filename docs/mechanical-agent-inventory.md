...
`fetch:roadmap` / `fetch:roadmap-body-check` / `fetch:<kind>` / `fetch:plan` / `fetch:wontfix` /
...
| `fetch:plan` | `rdm-wf-plan-review.js` | **byte-copied** `plan-review-driver` | no | the in-repo dispatch orchestrator only — the `implementation-plan` branch this site lives in is reached solely by `rdm-dispatch-phase`'s step 6; the local `rdm-plan-review` shim never targets an implementation plan | **not a new hoist candidate** | Added by `agent-orchestrated-dispatch` phase 32 to close the defect `fetch:<kind>`'s own row above records phase 30 causing: with the item-document read gone, the orchestrator became the ONLY thing that could supply the plan body, and had to reproduce it verbatim in its own `Workflow(...)` call arguments (19,717 characters on the first real dispatch that hit it). `planText` was **already** its own pre-existing hoist — a non-empty value skips this read entirely, byte-identical to the prior behavior — so this is not a second hoist over the same input; there is nothing further to add. What changed is that `planText` is now OPTIONAL when `planSlug` is present, so the orchestrator stopped supplying it and this site now fires on every dispatch. One bounded retry, `RAW_STDOUT_SCHEMA` verbatim-transcription contract, `extractPlanFromJson` rejects unless the fetched `json.slug` equals `planSlug` — the same slug the persist ref (`'plan/' + planSlug`) is built from — and fails CLOSED (`escalated`, `fetchError: true`, nothing persisted) on a second untrustworthy or mismatched payload, rather than grading a body that might not be the recorded plan. | partly (distributed-caller path) |
...
| `rdm-wf-plan-review.js` | `fetch:roadmap`, `fetch:roadmap-body-check`, `fetch:roadmap-intent`, `fetch:<kind>`, `fetch:plan`, `fetch:wontfix`, `gate:clear-tag:<kind>:<ident>` | **byte-copied** — inside the `plan-review-driver` block; edit `lib/plan-review.mjs` first, then copy verbatim. `verify-workflow-review.sh` §5b-drift gates the pair |
