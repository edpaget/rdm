...
- Prebuilt binaries for five target platforms — `aarch64-apple-darwin`,
  `x86_64-apple-darwin`, `x86_64-unknown-linux-gnu`,
  `aarch64-unknown-linux-gnu`, and `x86_64-pc-windows-msvc` — generated by
  cargo-dist on every release. Releases now also publish a PowerShell
  installer (`rdm-cli-installer.ps1`) alongside the existing shell
  installer and Homebrew formula, so Windows users can install with
  `irm <url> | iex`.
