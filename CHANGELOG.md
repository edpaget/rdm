...
- **Breaking for MCP clients:** `rdm_status` now returns an object instead of a bare array — a bare array cannot carry the `others`/`unattributed` attribution an agent needs to tell its own edits from a concurrent session's. The shipped shape is `{changes, others, unattributed}`; an `INDEX.md` a session wrote appears in `changes` like any other authored path. The `rdm_status` / `rdm_commit` / `rdm_discard` tool descriptions no longer mention generated indexes or a `(plus N …)` suffix.
