...
- **Code review no longer blocks on a deferral a phase or task declared in its own body.** The `ac` dimension's severity contract used to flag ANY criterion the target deferred, caveated, or shipped with acknowledged gaps as a blocking finding, with no distinction between a gap the author scoped out up front (per `docs/authoring-grammar.md`'s authoring rubric — e.g. an "Out of scope: ..." line in the Approach section) and a caveat an implementer invents at the end of a diff to excuse incomplete work. A deferral recorded in the target's own body is now honored as a bounded criterion, rated PASS as scoped; a deferral or caveat that first appears in the implementation diff or its commentary, with no antecedent in the body, still blocks exactly as before. This applies to code-mode review only; plan-mode review is unaffected.
