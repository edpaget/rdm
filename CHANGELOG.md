...
- **A review whose comment anchors ALL degraded at persist time is now distinguishable, by the caller, from an ordinary clean persist.** The persist ladder's build-time degradation clause already named the count and reason in the review's own summary, but nothing machine-readable told the calling skill to treat the run differently. `rdm-wf-review-refute-fix`'s standalone code-review path now also returns `persistDegraded: { requested, degraded, all }` alongside `persistCommands`/`persistScript`, and the persist ladder itself prints a trailing `anchorsDegraded=<all|partial|none>` line. `rdm-dispatch-phase` now parks `blocked` when `persistDegraded.all` is `true` — a review that recorded no usable per-file anchors at all should not be reported as ordinary successful persistence — while a partially-degraded run still proceeds normally.
