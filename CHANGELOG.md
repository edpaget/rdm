...
- A diverged `rdm remote pull` is no longer wedged by rdm's own backfilled merge mapping. Because rdm writes `.gitattributes` on every repo open, a repo predating the mapping became dirty through no act of the user's, and the pull refused with "cannot pull with uncommitted changes — commit or discard first" — an instruction that could not be followed, since `rdm discard --force` re-ensures the mapping and the tree could never come clean. The pull now restores that one file to `HEAD` for the duration of the merge and re-ensures it afterwards. The carve-out is byte-exact: a `.gitattributes` you edited yourself still blocks the pull and is never discarded on your behalf.
