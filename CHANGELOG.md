...
- A project can now hand its own development discipline — mutation testing, coverage floors, fuzzing, a house error-message style, a review emphasis — straight to the agents that do the work, without rdm growing a flag per discipline. rdm reads the prose a project already writes for its agents (`.claude/rules/**/*.md`, `AGENTS.md`, `.cursor/rules/*.mdc`, `.clinerules` as either a file or a directory, `.windsurf/rules/**/*.md`, and `.github/copilot-instructions.md`; `CLAUDE.md` is deliberately excluded because every subagent already loads it) and reproduces it **verbatim** inside the dispatched implementer's prompt and every reviewer prompt of the dispatch lane — the plan review that runs before any code is written as well as the code review that runs after — never summarized, re-worded, or reflowed. A `role:` frontmatter key addresses a rule to the implementer, the reviewers, or both (the default), and a `paths:`/`globs:` key scopes it to the files a change actually touches, so a Rust-only rule does not reach a shell-script change. A new repo-only `dispatch.directives` config key names the sources explicitly and **replaces** that discovery entirely rather than adding to it, with an explicitly empty value meaning "this project declares no directive sources". The read-only `rdm dispatch directives [--dir <path>] [--role <role>] [--format json]` command shows exactly what would be injected; its two reads fail in opposite directions on purpose — an unreachable plan repo falls back to discovery rather than erroring (a downstream consumer may have no `rdm.toml` at all), while a `--dir` that does not exist or names a file is rejected with an actionable error rather than reported as an empty result indistinguishable from a healthy project with no directives. The set is bounded at 8000 bytes per source and 16000 bytes total (paid once per dispatched agent), and an over-bound source is skipped **whole** rather than truncated — a rule cut mid-sentence can invert its own meaning — with every skip reported in the command's output, in a notice the agent itself sees, and in the dispatch outcome summary. Having no directives at all is normal: nothing is injected, nothing is reported, and no prompt changes by a byte. Reading stays inside the project: rdm never follows a symlink out of the scanned tree, including one at a source location's own root — a `.claude/rules` (or a declared directory entry) that is itself a link elsewhere resolves to nothing rather than splicing files from the rest of the machine into an agent's prompt. The same containment applies to a declared source that needs no symlink to escape: an absolute path, or one reaching upward with `..`, is refused before it is read and reported as `declared source escapes the scanned tree` — kept distinct from a plain `declared source not found` so a containment refusal is not mistaken for a typo. A declared entry may name a file, a directory, or a wildcard (`docs/rules/*.md`, `docs/rules/**/*.md`, or a bare `*`/`**` for every extension). This is guidance, not enforcement — a directive never becomes a finding, a gate, or a status, and it can never narrow a review. See `docs/project-directives.md`.
