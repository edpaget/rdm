...
- **`phase update`/`task update --start-commit` never silently drops a value it couldn't resolve or write.** A malformed value (not a full 40-lowercase-hex-character commit SHA) is refused before any resolution is attempted, naming the rejected value. A well-formed value that doesn't resolve to a commit in the item's worktree (or an explicit `--source <path>`, when given) is refused naming both the value and the repository it was checked against. Either way the status update — if one was also requested in the same call — is refused too, and nothing is written.
