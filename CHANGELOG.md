...
- **A phase or task's `reviewed` review is now scoped to its own commits, not every earlier phase's in a shared roadmap worktree.** `rdm phase update`/`task update --status in-progress` records the item's starting commit (`started_head`, write-once — a later `in-progress` re-stamp, including a `reviewed → in-progress` rework, never moves it), exposed by `phase show`/`task show --format json`. An explicit `--source <path>` on that same `in-progress` transition records that checkout's HEAD directly, without requiring a non-empty committed diff or (for a task) an explicit `--base` — both of which a fresh checkout at item start never has. `rdm review source` and the gated `reviewed` write now default their review `base` to the recorded `started_head` instead of the merge-base with the default branch; with no `started_head` recorded, `base` falls back to the merge-base as before and the response's new `baseNote` field says so. An explicit `--base` still overrides.
