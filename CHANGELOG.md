...
- **`parseCommentHeader` now parses legacy 6-key review comment headers, fixing repeat-finding detection in plan review.** Before `inScope` was added (phase 35 of this roadmap), persisted review comments carried only six header keys. `parseCommentHeader` required 7 or 8 keys and returned `null` for anything shorter, silently reclassifying every pre-existing 6-key comment as human-written — which defeated `priorFindingsFromReviews`'s repeat-finding detection in the plan-review round loop and mislabeled repeated findings as fresh. The parser now progressively tries 8-key, 7-key, and 6-key matches, returning an object (not `null`) on any match. A 6-key body parses with `inScope` and `anchor` both unknown (the established sentinels for "never graded" / "not written"), preserving the comment's original gating behavior unchanged.
