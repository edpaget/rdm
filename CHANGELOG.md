...
- **`--append-body` adds to a roadmap, phase, or task body without rewriting it.** `rdm phase update <stem> --append-body "$NOTE"` (and the same flag on `roadmap update` / `task update`) appends the text one blank line after whatever is already there. Until now `--body` was the only way to add a line, and it is whole-document-authoritative: a caller wanting to append had to read the current body, carry it, and hand the whole thing back — so a body could be silently truncated by a dropped line, and an agent-driven append meant a whole document crossing a model's output. `--append-body` needs none of that; it conflicts with `--body` and `--clear-body`, and appending nothing is a no-op rather than a clobber.
...
- **`phase update --append-body` now composes with `--expected-estimate-snapshot`.** The one estimate write carrying an optimistic-concurrency precondition was also the only one that could not append, so a guarded caller had to read the whole body out, carry it, and hand it back — the clobber-shaped write `--append-body` was added to remove. rdm-core already supported it; only the CLI's conflict list stood in the way. The snapshot is checked against the stored body *before* the update applies, so a guarded append is exactly as safe as a guarded whole-body write. `--clear-body` is still excluded: a conditional estimate that erases the body it was conditioned on has no coherent meaning.
...
- **`rdm verify run --item` now resolves a phase to its roadmap's shared worktree instead of never matching.** Under one-worktree-per-roadmap, a registered worktree is keyed by its bare roadmap slug, but `--item <roadmap>/<phase>` matched on the per-phase key and always fell into the "no worktree" arm — whose suggested remedy, `rdm worktree add <roadmap>/<phase>`, would have created exactly the per-phase worktree this project abandoned. `--item` now routes through the same roadmap-collapse policy the `reviewed` gate's worktree probe already uses, and additionally accepts the kind-prefixed `phase/<roadmap>/<stem>`, `roadmap/<slug>`, and `task/<slug>` grammar that `--on`/`--implements` use, alongside the existing unprefixed `rdm worktree add` grammar. A miss now suggests `rdm worktree add <roadmap>` (or `task/<slug>`), never a per-phase worktree.
