...
- **`max_refutations` is now project-scopable.** `rdm config set max_refutations N --project <name>` writes `[projects.<name>] max_refutations`, and `rdm config get max_refutations --project <name>` / `rdm config list --project <name>` report it with source `project config`. The budget resolves `RDM_MAX_REFUTATIONS`, then `[projects.<name>]`, then the plan-repo-wide value, then the global config; a project `0` still means "grade nothing" and does not fall through. The `rdm-dispatch-phase`, `rdm-review` and `rdm-plan-review` skills and the Codex runtime now pass `--project` on their `config get max_refutations --raw` call, so the reviewed project's budget applies. `rdm config list` now shows a malformed `RDM_MAX_REFUTATIONS` in the shared `(error: …)` row form instead of `(invalid: …)`. rdm-core API: `config::resolve_max_refutations` now takes `(project, repo, global, env)` and returns the resolved value with its source.
- **`default_branch` now resolves per project.** The post-commit `Done:` hook's branch filter, `rdm phase update`/`task update` (the needs-review "nothing to review" warning and `--source` binding), `rdm review source`, the `change/` review merge-base and `rdm info` read `default_branch` for the command's project, so `[projects.<p>] default_branch = "develop"` serves a project whose trunk is `develop`. The hook takes its project from `RDM_PROJECT`, then `default_project`; with neither it uses the plan-repo-wide value, and a commit on a non-default branch still skips cleanly. The project frontmatter's `source.default_branch` still wins for the `change/` merge-base. These consumers now honor `RDM_DEFAULT_BRANCH`, which `rdm info` previously ignored. rdm-core API: `config::resolve_default_branch` is new.

- **The `reviewed` transition gate and the `needs-plan-review` stamp now honor `[projects.<p>]` overrides.** `rdm phase update`/`task update --status reviewed` and rdm-server's `PATCH` to `reviewed` resolve `gates.reviewed` for the item's project (`rdm config set gates.reviewed true --project <p>` gates one project only), and `rdm roadmap|phase|task create` resolve `plan_review` for the project the item is created in. `RDM_REVIEWED_GATE` and `RDM_PLAN_REVIEW` still win over every config layer. One behavior change applies even with no `[projects]` table: the gate now also honors the generic `RDM_GATES_REVIEWED` (below `RDM_REVIEWED_GATE`, which wins when both are set), so `RDM_GATES_REVIEWED=true` enables the gate and an invalid value such as `yes` fails every `phase update`/`task update` and every rdm-server phase/task `PATCH` — not only `reviewed` writes — with an error naming the variable, exactly as an invalid `RDM_REVIEWED_GATE` already did, because the gate is resolved on every update. rdm-core API: `config::resolve_reviewed_gate` now takes `(project, repo, env)` instead of `(env_value, config)`, `config::reviewed_gate_enabled_at` takes the project, and `config::resolve_plan_review` is new.
...
- **`review-consolidate` model step.** `rdm model resolve review-consolidate` resolves the model and reasoning effort for the review pipeline's new consolidator. It defaults to the `large` tier, is clamped up to `[models] review_floor` like the other review steps, and can be overridden under `[models.steps]` (`review-consolidate = "<tier>"`). `rdm model show` lists it after the existing steps.
- **A persistent, overridable refutation budget.** The new `max_refutations` config key (repo or global, `rdm config set max_refutations N`) and the `RDM_MAX_REFUTATIONS` environment variable set how many consolidated review units (distinct defects) a review run grades with a refuter. `0` is a real value meaning "grade nothing", not "unset". `rdm-do --auto`, `rdm-dispatch-phase`, `rdm-autopilot`, `rdm-review` and `rdm-plan-review` accept `--max-refutations N`, and the skills that call a review engine read the config key when no flag is given. The Codex runtime resolves the budget the same way (spec, then `RDM_MAX_REFUTATIONS`, then config) and records the result in its run journal. Precedence: `docs/file-formats.md` § "`max_refutations` precedence".

### Changed

- **The review pipeline now merges duplicate findings before refuting them.** When several reviewers report the same defect, one consolidator agent groups their findings, so the defect uses one refuter and one refutation-budget slot instead of one per duplicate. The agent only assigns finding ids to groups and never rewrites a finding. Each merged finding keeps the text of its highest-confidence member, takes the most severe member's severity and the highest member confidence, and carries `concerns`, `mergedFrom` and `clusterWhy`. A finding with no duplicate comes through unchanged. If the consolidator fails or returns an invalid grouping, the review continues with every finding separate, as before. The review `budget` gains `consolidated`, `collapsed` and `clustering` (`{ ran, retried, failedOpen, reason }`). `produced` still counts the raw findings. The review engines, `rdm-dispatch-phase`, `rdm-review`, `rdm-plan-review` and the Codex runtime accept the resolved `consolidateModel`/`consolidateEffort` and pass them to the consolidator.
- **A merged review finding now says what was merged and why.** A persisted review comment for a finding the consolidator merged names the dimensions that reported it, the member finding ids and the consolidator's reason for grouping them; a finding with no duplicate is persisted exactly as before. The review OUTCOME's `reviewBudget` carries `consolidated`, `collapsed`, `clustering` and `everFailedOpen` (true when any round's consolidator failed open), and the standalone code review's summary names a collapse (` [consolidated: N findings into M units]`) or a failed consolidator (` [consolidation failed open]`). The `rdm-review` and `rdm-plan-review` skills replace their "dedup" instruction with the same consolidation step: group findings that are one defect after the finders report, refute each group once, and merge by the same rule.
...
- **`rdm cost report --roadmap <slug>` reports what a roadmap cost and which phases dominated.** It joins every run record of the roadmap to its Claude Code session by time window: an `Agent` subagent or Workflow agent counts toward the phase whose unit window contains the main-session line that launched or returned it, and the main session's own turns count by their own timestamps, so the orchestrator's spend inside a unit counts toward that phase. It prints roadmap totals per model and token class; one row per phase with tokens by class, attempts (a rework re-dispatch adds to the same phase) and wall clock, ordered by total tokens; the run overhead spent between units; itemised unattributed spend (errored Workflow agents, units that never ended, and sources with no anchor, listed once per session); and a count of the spend in the same sessions that falls outside the roadmap's runs, which is excluded from every total. Each session is read once, even when two runs share it. An open run is labelled incomplete, a run whose session is gone is reported as missing while the rest of the report prints, and a run with no session is reported as unjoinable. `--format json` gives the same data as structured output. Tokens only. See `docs/run-accounting.md`.

- **`rdm run` records what an autonomous-lane run drove, in which session, and when.** A run record (`projects/<project>/runs/<id>.md`) names the driver (`autopilot` or `dispatch-phase`), the roadmap or task it drove, the invocation arguments, the Claude Code session it ran in, its start and end times, its stop reason and its status (`open`, `closed` or `abandoned`), plus one entry per dispatched unit with the phase stem (or task slug), an attempt ordinal (a rework re-dispatch is attempt 2), the outcome, and start and end timestamps. Six commands: `rdm run record` (prints only the new id, so `id=$(rdm run record …)` captures it), `rdm run unit-start` / `rdm run unit-end` around each dispatched unit, `rdm run close --stop-reason … [--status abandoned]`, and `rdm run list [--roadmap <slug> | --task <slug>]` / `rdm run show`, each with `--format json`. The session is `--session-uuid` if given, else the raw `CLAUDE_CODE_SESSION_ID`, else none — and a run with none says it cannot be joined to spend. A run never closed still loads and is labelled incomplete, as is a unit that never ended; a run whose roadmap was later deleted still loads. Every write is staged like any other rdm mutation. Runs are not included in `rdm search`; use `rdm run list`. `rdm describe run` documents the format.

- **Autopilot and dispatch runs now write run records.** `rdm-autopilot` opens a run with `rdm run record` before doing anything else, passes its id to every `rdm-dispatch-phase` unit as `--run <id>`, and closes it with the stop reason on every stop path, committing `chore(plan): close run <id>`. `rdm-dispatch-phase` records a `unit-start`/`unit-end` entry per unit, so a rework re-dispatch shows up as attempt 2. Dispatched standalone or through `rdm-do`, it opens and closes its own single-unit run. Accounting is best-effort: a failed `rdm run` call logs a warning and never changes how the run proceeds. Applies to the shipped skills (`rdm agent-config claude --skills`, the `rdm` plugin) and this repo's dogfood copies.

- **`rdm cost` reports what a Claude Code session spent, in tokens.** `rdm cost --session <uuid>` prints the session's tokens per model and per token class (uncached input, output, 5-minute and 1-hour cache writes, cache reads), plus a per-source breakdown: the main session, each `Agent` subagent (nested ones included) and each Workflow agent, each with the main-transcript timestamp it is anchored to. `rdm cost --workflow-run <wf_id>` narrows the report to one Workflow run's agents, and a bare `rdm cost` reports the current session from `CLAUDE_CODE_SESSION_ID`. `--format json` gives the same report as structured data. The session is found under any project directory, worktree ones included. A request counted by two sources is counted once. Sources that cannot be anchored are still counted and listed as unanchored. Errored and cached Workflow agents are flagged. A malformed Workflow sidecar is skipped with a warning naming its path. The sidecar's `totalTokens` is shown but never counted. It reads only local files under `$CLAUDE_CONFIG_DIR` (or `~/.claude`), needs no plan repo, and reports tokens, not dollars. See `docs/run-accounting.md`.

- **`rdm-core` now exposes per-model, per-token-class token accounting over Claude Code transcript text** (`rdm_core::usage`). `parse_transcript` turns already-read transcript lines into deduped per-request usage across five token classes (uncached input, output, 5-minute and 1-hour cache writes, cache reads), deduping by `requestId` with the last line winning and the first position and timestamp kept. A request whose final usage is all zero is excluded and reported as a `UsageWarning` rather than counted as a zero observation. `UsageLedger` aggregates requests per raw `message.model` string, with request counts and totals, over all requests or only those inside a half-open `[start, end)` time window. Tokens only, no pricing. `rdm-devtools`' measurement instruments now use this implementation instead of their own.
...
- **The `RDM_HARNESS_*_BARRIER` test seams now announce when a process has parked, and the session, commit and race regressions run under `cargo nextest run`** (contributor-facing; ordinary use is unaffected — the seams stay inert unless a harness sets them). A process parked at `RDM_HARNESS_FLUSH_BARRIER`, `RDM_HARNESS_JOURNAL_BARRIER`, `RDM_HARNESS_APPEND_BARRIER` or `RDM_HARNESS_COMPACT_BARRIER` now creates the empty file `<marker>.parked` beside the release file it waits for, so a harness can wait for readiness instead of guessing from elapsed time. `scripts/verify-{session-identity,scoped-commit,lost-update,journal-truncation-race}.sh` are deleted; their scenarios, and their planted-mutation checks rebuilt from an isolated copy of the working tree, are the `rdm-cli` test binary `concurrency` (`cargo nextest run -p rdm-cli --test concurrency`).

- **The distribution, plugin, golden-JSON and CLI-loop verification scripts are now `cargo nextest run` tests** (contributor-facing; nothing shipped changes). `scripts/verify-{agent-config-distribution,plugin-distribution,plugin-install,plugin-loop,claude-code-web-loop,rdm-plan-fixture,golden-json,backlog-groom-loop,review-revision-loop,worktree-review-loop}.sh` and their shell libraries are replaced by the `rdm-cli` test binaries `distribution`, `cli_loops` and `golden_json`, which run every emission, generator and CLI loop in a sandboxed temp tree. `scripts/capture-golden.sh` is replaced by the ignored `golden_json::bless` case: re-bless with `cargo nextest run -p rdm-cli --test golden_json --run-ignored only -E 'test(=bless)'`.

- **The token and refuter measurement tools and the Codex coexistence check are now Rust commands in the repository-only `rdm-devtools` crate** (nothing is added to the shipped `rdm` binary). `node scripts/measure-lane-tokens.mjs`, `measure-refuter-severity.mjs`, `mine-refuter-corpus.mjs` and `run-refuter-agreement.mjs` become `cargo run -q -p rdm-devtools --bin rdm-measure -- lane-tokens|refuter-severity|mine-refuter-corpus|refuter-agreement` with the same flags and report schemas, except that each report's `instrument` field names the Rust command and `run-refuter-agreement --dispatch-stub <module>` is replaced by `--claude-bin <path>`. `refuter-severity --audit`, `refuter-agreement --audit`/`--score-only`/`--batch-power` and `mine-refuter-corpus` no longer need a JavaScript runtime; replaying the canonical review decisions still runs the real `review.mjs` (a new `--review-lib` flag points at another copy). `node scripts/verify-codex-coexistence.mjs <rdm> <codex> [auth.json]` becomes `rdm-smoke codex-coexistence --rdm <path> --codex <path> [--copy-auth-from <auth.json>]` with `--discovery-timeout-secs`/`--exec-timeout-secs`, exit codes 0/1/124/130/143, and live invocation reported NOT RUN without `--copy-auth-from`. Date windows (`--since`/`--until`) accept the ECMAScript ISO date-time forms and refuse other formats; a sidecar missing its `runId` no longer crashes `lane-tokens`. `scripts/verify-token-report.sh` and `scripts/verify-refuter-agreement.sh` are replaced by `cargo nextest run -p rdm-devtools` tests.
