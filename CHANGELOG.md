...
- **The `RDM_HARNESS_*_BARRIER` test seams now announce when a process has parked** (contributor-facing; ordinary use is unaffected — the seams stay inert unless a harness sets them). A process parked at `RDM_HARNESS_FLUSH_BARRIER`, `RDM_HARNESS_JOURNAL_BARRIER`, `RDM_HARNESS_APPEND_BARRIER` or `RDM_HARNESS_COMPACT_BARRIER` now creates the empty file `<marker>.parked` beside the release file it waits for, so a harness can wait for readiness instead of guessing from elapsed time.
