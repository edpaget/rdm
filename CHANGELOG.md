...
- **A phase or task's `reviewed` review is now scoped to its own commits, not every earlier phase's in a shared roadmap worktree.** `rdm phase update`/`task update --status in-progress` records the item's starting commit (`started_head`) — but only on the transition whose prior status was `not-started`/`open`, i.e. the item's first-ever entry into `in-progress`; a later `in-progress` transition (a `reviewed → in-progress` rework, a `blocked → in-progress` unpark, or an `in-progress → in-progress` re-stamp) never records a value, even when the field is still empty, so a resumed or reworked item can never have its base moved forward past commits it already made — exposed by `phase show`/`task show --format json`. An explicit `--source <path>` on that same first-entry transition records that checkout's HEAD directly, without requiring a non-empty committed diff or (for a task) an explicit `--base` — both of which a fresh checkout at item start never has. `rdm review source` and the gated `reviewed` write now default their review `base` to the recorded `started_head` instead of the merge-base with the default branch; with no `started_head` recorded, `base` falls back to the merge-base as before and the response's new `baseNote` field says so. An explicit `--base` still overrides.
...
- **`phase update`/`task update --status in-progress` no longer silently drops a starting HEAD it couldn't resolve.** On the item's write-once `started_head` stamp (its first-ever entry into `in-progress`), an explicit `--source <path>` whose HEAD can't be read (no git repository there, or no commits yet) is now a hard error naming the path — the status update is refused and nothing is written, rather than quietly succeeding with the field left unset forever. Automatic resolution (no `--source` given) stays best-effort — a resolution miss there still lets the transition succeed — but now prints a non-blocking warning to stderr naming the item, since the field can never be filled in on a later `in-progress` transition and the only prior trace was a `baseNote` on some later `review source` call.

- **`review source`'s `started_head` default no longer trusts a recorded value that is no longer an ancestor of HEAD.** If the roadmap or task branch was rebased onto an advanced default branch, or an earlier commit amended/rebased, a recorded `started_head` can still resolve as a commit object while no longer being reachable from the current checkout's HEAD — reviewing that range directly used to silently mix in every change the default branch picked up, plus reverse diffs of the rewritten commits. `started_head` is now additionally checked with `git merge-base --is-ancestor` before use; on a miss, `base` falls back to the merge-base with the default branch (a safe superset of the intended range) and `baseNote` names the stale value and explains why. An explicit `--base` is unaffected by this check.
