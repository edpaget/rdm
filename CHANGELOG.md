...
- The MCP server now exposes the LLM revision workflow, so an agent can
  ...
  - `rdm_review_address_comment` — flips a comment to `addressed` or
    ...
    clarification reply. When `applied_commit` is omitted with status
    `addressed`, the plan-repo HEAD *before* the call is recorded as a
    best-effort fallback — never for `wont-fix`.
  - `rdm_review_complete` — closes a review as `addressed`, refusing
    while any comment is open and listing the offending comment ids.
- MCP mutation responses (`rdm_phase_update`, `rdm_task_update`,
  `rdm_roadmap_update`, and the new `rdm_review_address_comment`) now
  report the plan-repo commit each call produced (a `Commit: <sha>`
  trailer line, or a `commit` JSON field), so agents can thread the exact
  edit commit into a review comment's `applied_commit` instead of
  guessing from a later HEAD. Omitted under staging mode, where nothing
