...
- Concurrent `rdm` mutations and commits that share one session id no longer lose journaled paths. Under an agent harness that publishes one session id for a whole session (`CLAUDE_CODE_SESSION_ID` and friends), every parallel subagent and MCP call resolves that same changeset, so a `rdm commit` and a `rdm task create` running at the same time were two separate processes writing the same journal — and the commit rewrote that journal wholesale, silently destroying anything recorded while it ran. With 40 parallel creates and 6 concurrent commits under one session, all 40 tasks landed but both `INDEX.md` files were left dirty, `rdm session list` reported the changeset as having journaled no paths, and the next `rdm commit` disowned those indexes as belonging to another changeset with no way to recover them short of `--all`. A commit now records what it landed by appending a single line rather than rewriting the file, and that record names the exact content it landed — so a path a concurrent process rewrote in the meantime (the regenerated `INDEX.md` files, in practice) keeps its entry and the next commit lands it normally. `rdm session gc` gains a second sweep that removes changeset journals which are fully committed and owned by no live session, and reports how many it removed; `rdm session list` no longer reports a changeset that claims no paths, matching what it showed before. That sweep is itself safe to run at any time, including from an unrelated shell while other processes are actively writing: rdm cannot generally tell whether anyone is still appending to a shared changeset (a session id pinned with `RDM_SESSION`, or published by a harness, records nothing on disk that would say so), so a write that lands in a journal the sweep has already rewritten is detected and redone against the live one rather than being trusted not to happen — and a write that keeps losing that race stops racing and takes the sweep's own lock, which makes the sweep skip, so a concurrent sweep can slow a write down but never cost it its record. That last guarantee no longer has a hole in it for a slow sweep: rdm's advisory locks are reclaimed after 30 seconds so a killed process can never wedge one forever, which meant a sweep held up past that (a loaded machine, a slow disk) could lose its lock while still running and then overwrite the very write that had just taken it. A sweep now re-checks that it still owns its lock immediately before rewriting, and abandons the rewrite if it does not, while a write made under the lock verifies for itself that it landed — so the two notice each other from both sides instead of one silently overwriting the other. The last writer sitting outside all of this was `rdm session discard <id> --force`, which destroys a changeset deliberately and did it by deleting the journal file outright — so a `rdm task create` running at the same moment under the same session id lost its record and was left on disk as unattributed dirt that no later commit would claim. A discard now retires exactly what it read, using the same appended record a commit writes, and removes the file afterwards only when nothing else is writing to it: a concurrent write keeps its record and commits normally, while an uncontended `rdm session discard --force` still leaves no journal behind.
...
- The MCP `rdm_commit` / `rdm_status` / `rdm_discard` tools are scoped the same way — one MCP server process is one session. **Breaking for MCP clients:** `rdm_status` gains a third array, `others`, listing paths another session left uncommitted that `rdm_commit` will not touch. The three tool descriptions previously promised whole-tree semantics ("Land every currently staged change", "Everything is restored") and have been rewritten to state what the tools actually do.
- **Every surface that tells an agent how to commit now describes the scoped model.** The instructions `rdm agent-config` emits for Claude, Codex/AGENTS.md, Cursor, Copilot and Pi previously stated outright that "`rdm status`, `rdm commit`, and `rdm discard` operate on the whole plan repo's git state" and that a commit "lands every staged change" — both false since scoping shipped, and read by an agent *before* it ever calls a tool. The CLI instructions now describe the changeset your commit lands, what `rdm status`'s three buckets mean, `RDM_SESSION`, the `rdm session id` / `list` / `journal` observation surface, and the `--all` / `--changeset <id>` escape hatches; the MCP instructions now match the tool descriptions, including `rdm_status`'s `{changes, generated, others}` shape. Batching guidance is corrected rather than dropped — batching a related group of mutations into one `rdm commit` is still right, and is now safe under concurrency. The `rdm-revise` skill's "commit per comment" rule keeps its conclusion with a corrected premise (it lands every change in *your own* changeset). The runtime hint printed after each mutation now reads `(staged in this session's changeset — run \`rdm commit\` to persist)`, so the first place an agent meets the concept no longer implies a whole-tree sweep. The shipped plugin tree at `plugins/rdm/` is regenerated to match.
...
- **Breaking for direct `rdm-store-git` library consumers:** `GitStore::commit_now` is replaced by two explicitly-named entry points — `commit_changeset` (the scoped default) and `commit_whole_tree` (the machine-global escape hatch) — and `GitRepo::git_commit` is now `pub(crate)`, so an out-of-crate caller that tries to sweep the tree fails to compile rather than drifting in silently. `StatusReport` gains a third `others` bucket alongside `user`/`derived`, filled by one partition rather than by a second filter stacked on the first; `is_clean()` still means "nothing at all differs" and a new `is_changeset_clean()` is the scoped gate. `GitRepo::ensure_gitattributes` now reports whether it wrote, so the store can journal the back-fill and that write can still reach a commit. `rdm.toml` is written through the store via the new `rdm_core::io::save_config` rather than by a raw filesystem write, so `rdm init --remote` still lands its config commit.

- `rdm status`, `rdm commit`, `rdm discard`, the post-command "uncommitted changes" hint, and the MCP `rdm_status` / `rdm_commit` / `rdm_discard` tools now **count only changes you authored**, and report the regenerated `INDEX.md` files separately. Previously every one of them lumped derived output in with your own edits, so after any mutation a session could not tell what its next `rdm commit` would actually sweep up — a read-only `rdm list` would report "3 uncommitted change(s)" when you had edited one file. `rdm status` now prints `1 file(s) changed` plus a line naming the generated files (`(2 generated index file(s) will be included in the next commit: INDEX.md, projects/demo/INDEX.md)`), and `rdm commit` / `rdm discard` report `… 1 file(s) (plus 2 regenerated index file(s))`. Nothing is hidden and nothing is dropped: the generated indexes are still written into every commit and still restored by every discard. Only the *reporting* narrows — `rdm commit` and `rdm discard` remain gated on whether anything at all differs from HEAD, so a tree holding nothing but regenerated indexes is still committable (otherwise it would stay dirty forever and `rdm remote pull` would refuse to run). A `rdm status` saying `No uncommitted changes.` can therefore still be followed by a `rdm commit` that lands index files; that case now prints `Committed 2 regenerated index file(s).` A hand-customized `.gitattributes` is deliberately **not** treated as generated — it is a real, committable file you may have edited, so it keeps showing up in `rdm status`.
- **Breaking for MCP clients:** `rdm_status` now returns an object `{"changes": [{path, change}, …], "generated": ["INDEX.md", …]}` instead of a bare array. A bare array cannot carry the split, and an agent that cannot see the split is exactly the failure this change exists to fix. All three git tool descriptions now state that generated indexes are excluded from `changes` but are still written into the commit `rdm_commit` creates, so an agent does not conclude its index regeneration was dropped.
- **Breaking for direct `rdm-store-git` library consumers:** `GitRepo::git_status` is replaced by `GitRepo::git_status_report`, which returns a `StatusReport { user, derived }` carrying the contract "gate on `is_clean()`/`total()`, report on `user`". The raw unfiltered walk is now `pub(crate) git_status_all` — deliberately unreachable from outside the crate, so no future call site can silently reacquire a list that conflates generated output with user edits. The membership rule lives in core as `rdm_core::paths::is_derived_path`, defined by round-tripping against `paths::index_path` / `paths::project_index_path` so it cannot drift from what `ops::index::generate_index` writes. `StatusReport` also owns the summary wording every interface prints — `commit_summary()` and `discard_summary()` — so `rdm commit` / `rdm discard` and the MCP `rdm_commit` / `rdm_discard` tools render the same report identically by construction rather than by convention.
