...
- **`INDEX.md` and `projects/<p>/INDEX.md` are now ordinary files, with no special status anywhere in rdm.** rdm no longer has a notion of a "generated" or "derived" path at all. `rdm status` lists a dirty index as a plain change instead of on a separate `(N generated index file(s) will be included in the next commit: …)` line; `rdm commit` reports `Committed N file(s).` with no `(plus N regenerated index file(s))` suffix; `rdm discard --force` likewise counts every path it restored once, with no split. The post-command `(N uncommitted change(s))` hint counts them too, so a plan repo carrying a stale tracked `INDEX.md` left over from before this roadmap now sees it in that count and in `rdm status` — which is the honest answer for a file nothing maintains any more (a later phase adds an explicit prune).

  A commit is also no longer rebuilt in memory: previously a scoped commit regenerated the indexes from HEAD-plus-your-changeset rather than taking them off disk, which bought a deliberate one-directional `tree ⊇ index` divergence — a document could land in the tree with no index row for it when its project belonged to another uncommitted session. That mechanism is gone, and the divergence **ceased to exist** rather than being maintained: a commit contains exactly the bytes some session wrote, so there is no reconstructed index for the tree to diverge from. A commit under a project another session has not landed yet still lands, as before, but now because nothing at commit time reads a project — not because an orphan-subtree prune defends against it.

  One consequence you can hit: a regenerated index is no longer exempt from the overwrite guard. If you ran `rdm index` (now removed, see *Removed* below) and another session rewrote the same file before you committed, your `rdm commit` refused with the ordinary "changed by another session" error naming the path, instead of silently reconciling it away. This is the correct lost-update behavior for a file rdm no longer owns.
...
- **Breaking for MCP clients:** `rdm_status` no longer returns a `generated` array. Its response is `{changes, others, unattributed}`; an `INDEX.md` a session wrote appears in `changes` like any other authored path. The `rdm_status` / `rdm_commit` / `rdm_discard` tool descriptions no longer mention generated indexes or a `(plus N …)` suffix.
...
- **`INDEX.md` and `projects/<p>/INDEX.md` are no longer regenerated on every mutation.** A commit now contains only the files you authored: `rdm status` after a single `rdm task update --status` names exactly one path, `rdm commit` reports `Committed 1 file(s).` with no regenerated-index suffix, and `rdm discard --force` leaves the generated indexes on disk exactly as it found them — unless *your own* session regenerated one with `rdm index` (now removed, see *Removed* below), in which case it was part of your changeset and was restored along with everything else you wrote, so a discard never stranded a half-updated index nobody owned. (This entry originally described that restore as reported on a separate `(plus N generated index file(s))` line; the generated-path removal entry above supersedes that — every restored path is now counted once.) A generated index was no longer exempt from the discard's overwrite guard either: if another session had rewritten one since you regenerated it, your discard left it alone and reported it as skipped instead of reverting their rows out from under them. This takes the plan repo's central write-contention object off the write path — two sessions mutating different files no longer both write the same two derived files. *(Superseded within this same unreleased cycle: `rdm index`, and both `rdm remote pull` and `rdm resolve`'s post-merge/post-pull regeneration, are all removed below — nothing refreshes an index any more, on demand or otherwise. Existing repos keep their tracked `INDEX.md` files untouched.)* `rdm init` no longer seeds an `INDEX.md`, since nothing maintains it.
...
- The MCP `rdm_commit` / `rdm_status` / `rdm_discard` tools are scoped the same way — one MCP server process is one session. **Breaking for MCP clients:** `rdm_status` gains an `others` array, listing paths another session left uncommitted that `rdm_commit` will not touch. The three tool descriptions previously promised whole-tree semantics ("Land every currently staged change", "Everything is restored") and have been rewritten to state what the tools actually do.

- **Every surface that tells an agent how to commit now describes the scoped model.** The instructions `rdm agent-config` emits for Claude, Codex/AGENTS.md, Cursor, Copilot and Pi previously stated outright that "`rdm status`, `rdm commit`, and `rdm discard` operate on the whole plan repo's git state" and that a commit "lands every staged change" — both false since scoping shipped, and read by an agent *before* it ever calls a tool. The CLI instructions now describe the changeset your commit lands, what `rdm status`'s three buckets mean, `RDM_SESSION`, the `rdm session id` / `list` / `journal` observation surface, and the `--all` / `--changeset <id>` escape hatches; the MCP instructions now match the tool descriptions, including `rdm_status`'s response shape (`{changes, others, unattributed}` as shipped, after the generated-path removal above). Batching guidance is corrected rather than dropped — batching a related group of mutations into one `rdm commit` is still right, and is now safe under concurrency. The `rdm-revise` skill's "commit per comment" rule keeps its conclusion with a corrected premise (it lands every change in *your own* changeset). The runtime hint printed after each mutation now reads `(staged in this session's changeset — run \`rdm commit\` to persist)`, so the first place an agent meets the concept no longer implies a whole-tree sweep. The shipped plugin tree at `plugins/rdm/` is regenerated to match.
...
- **Breaking for MCP clients:** `rdm_status` now returns an object instead of a bare array — a bare array cannot carry the `others`/`unattributed` attribution an agent needs to tell its own edits from a concurrent session's. *(It briefly also carried a `generated` array; that was removed later in this same unreleased cycle along with the generated-path concept, so the shipped shape is `{changes, others, unattributed}`.)*

- **Breaking for direct `rdm-store-git` library consumers:** `GitRepo::git_status` is replaced by `GitRepo::git_status_report`, which returns a `StatusReport` carrying the contract "gate on `is_clean()`/`total()`, report on `user`". The raw unfiltered walk is now `pub(crate) git_status_all` — deliberately unreachable from outside the crate, so no future call site can silently reacquire a list that conflates one session's dirt with another's. *(The report also briefly carried a `derived` bucket backed by `rdm_core::paths::is_derived_path`; both were removed later in this same unreleased cycle.)* `StatusReport` also owns the summary wording every interface prints — `commit_summary()` and `discard_summary()` — so `rdm commit` / `rdm discard` and the MCP `rdm_commit` / `rdm_discard` tools render the same report identically by construction rather than by convention.
...
- Plan repos now gain the `INDEX.md` merge-driver mapping **with no user action**, wherever they came from. Only `rdm init` ever wrote the `.gitattributes` half, so a repo that was cloned, or created before the merge driver shipped, or opened by any other path, silently lacked it and fell back to a three-way text merge on generated index files. Both halves — the `.gitattributes` entries in the worktree and the `[merge "rdm-index"]` section in `.git/config` — are now ensured on every repo open and on every clone (`rdm init --remote`, `rdm bootstrap`), best-effort: against a read-only checkout rdm warns and continues rather than failing the command. There is still no setup command to run. The newly written `.gitattributes` shows up as an added file until your next `rdm commit`; that is intended — it must be committed for the mapping to travel with clones.

- `rdm discard --force` no longer silently un-maps the repo from the `INDEX.md` merge driver. The mapping is re-ensured immediately after the working tree is restored, covering both shapes of the bug: an `.gitattributes` that was not yet committed (previously deleted outright) and one whose committed version predates the mapping (previously reverted to a version without it). The MCP `rdm_discard` tool inherits the fix.

- `rdm remote pull` is no longer wedged by rdm's own backfilled merge mapping — on either of its two paths. Because rdm writes `.gitattributes` on every repo open, a repo predating the mapping becomes dirty through no act of the user's. A **diverged** pull refused with "cannot pull with uncommitted changes — commit or discard first", and a plain **fast-forward** — the commoner case, and the one `rdm bootstrap` / `rdm init --remote` take — failed inside git with "the following untracked working tree files would be overwritten by merge: .gitattributes" the moment any peer had committed its own copy. Both were unrecoverable: `rdm discard --force` re-ensures the mapping, so the tree could never come clean and every retry failed identically. The pull now restores that one file to `HEAD` for the duration of the merge and re-ensures it afterwards, on both paths. The carve-out is byte-exact: a `.gitattributes` you edited yourself still blocks a diverged pull and is never discarded on your behalf. A fast-forward's tolerance of unrelated uncommitted edits is unchanged — that judgment is still git's.

- `rdm discard --force` no longer reports `removed:  .gitattributes` for a file it puts straight back. Because the discard re-ensures the merge mapping after restoring the tree, that line was false the moment it was printed; the file is now reported as `reinstalled: .gitattributes (rdm-managed)`.
