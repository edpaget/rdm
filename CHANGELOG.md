...
- **`rdm verify run --item` no longer misreads a malformed kind-prefixed reference (e.g. `phase/<roadmap>` with the stem omitted) as a phase of a roadmap literally named `phase`.** It is now refused up front with the same grammar-naming message `plan/`/`change/` already get, instead of the garbled nested "unknown item" error `ItemRef::parse` used to produce. A roadmap genuinely named `roadmap` or `phase` is unaffected — neither is a reserved roadmap slug, so it still resolves through the ordinary `<roadmap>/<stem>` grammar.
