...
2. Invoke the `rdm-wf-dispatch-phase` Workflow (`.claude/workflows/rdm-wf-dispatch-phase.js`) via the `Workflow` tool with `{ task: <slug>, alreadyInProgress: true, rdmBin, project: {proj_param} }`; block for its returned OUTCOME. The task-mode OUTCOME is keyed by `task` (the slug), not `roadmap`/`phase`. `rdmBin` is the exact rdm executable the workflow's own Bash agents invoke and is **REQUIRED** even on MCP — the workflow shells out even though this shim does not, so omitting it hard-breaks the lane on first dispatch. `project` is optional and applies only to project-scoped commands.
