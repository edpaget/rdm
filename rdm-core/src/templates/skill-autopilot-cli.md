...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - **Park**: run `rdm phase update S --status blocked --reason "<reason>" --no-edit --roadmap <slug> {proj_flag}`, then read it back with `rdm phase show S --roadmap <slug> --format json {proj_flag}` and confirm `status: blocked`. Retry up to **2** times total. Whether or not the read-back ever confirms, append `{ stem: S, reason }` to `escalations`, set `stopReason: escalated`, and go to step 4 (stop) — do **not** continue the loop, regardless of whether the read-back confirmed. An unconfirmed park write must never abort the run before it can print its summary; log a loud warning in that case instead (the plan-repo status may not reflect the park, but the escalation is still recorded here). The parked phase's own commits stay right where they are, on the shared `roadmap/<slug>` branch; because this run stops here, nothing is dispatched on top of them in *this* run. A human who resolves the park and re-invokes autopilot (or `rdm-dispatch-phase` directly) resumes from `rdm next` exactly as before.
...
escalations awaiting review (<n>): <"none", or the block below>
  - <stem> [<stage>]: <reason>
  ...
review the queue: rdm review blocked {proj_flag}
