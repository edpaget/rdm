...
- `project` — the optional project name following `--project` (the name in `{proj_flag}`). Omitted → the `<proj-flag>` placeholder used everywhere below renders as nothing (no `--project` flag at all, never an empty value), and rdm's own `RDM_PROJECT`/`default_project` chain applies.
