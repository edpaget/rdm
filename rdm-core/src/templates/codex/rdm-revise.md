...
```bash
"${RDM_BIN:-rdm}" review requests --format json {proj_flag}
"${RDM_BIN:-rdm}" review show <review-id> --format json {proj_flag}
```
...
```bash
"${RDM_BIN:-rdm}" review update <review-id> --comment <n> --status addressed --applied-commit <plan-sha> --reply "What changed" {proj_flag}
```
