...
```bash
"${RDM_BIN:-rdm}" roadmap list {proj_flag}
"${RDM_BIN:-rdm}" search <topic> {proj_flag}
"${RDM_BIN:-rdm}" tag list {proj_flag}
```
...
```bash
"${RDM_BIN:-rdm}" roadmap create <slug> --title "Title" --body "Intent and scope" --tags <topic>,needs-plan-review --no-edit {proj_flag}
"${RDM_BIN:-rdm}" phase create <bare-slug> --number 1 --roadmap <slug> --title "Title" --body "Work and acceptance criteria" --tags <topic>,needs-plan-review --no-edit {proj_flag}
```
