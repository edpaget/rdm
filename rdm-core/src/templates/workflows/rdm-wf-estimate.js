...
// a parallel() fan-out, and hands back ONE writeback command per phase, which
// persists the difficulty and nothing else. Already-estimated phases are
// skipped, which makes a re-run idempotent.
//
// Estimation writes NOTHING to the item body: no audit note, no body read, no
// body write. A rater's justification is reported in the summary and persisted
// nowhere.
//
...
export const meta = {
  ...
  description:
    "Rate an rdm roadmap's unestimated phases: list -> filter -> parallel-rate -> write back the difficulty only (tier derives in core, nothing is written to the body), skipping already-estimated phases",
