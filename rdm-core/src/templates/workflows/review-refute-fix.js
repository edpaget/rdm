...
if (mode === 'plan' || !(isTask || hasPhaseIdentifiers)) {
  ...
  // runReview resolves { survivors, acTable } now; this legacy path's EXTERNAL
  // { mode, survivors } contract stays byte-for-byte the same — only this
  // internal destructure changes. acTable is discarded (irrelevant to the
  // legacy shape and always null in plan mode).
...
if (changedFiles.length === 0) {
  ...
  const result = await runReview({ target: reviewTarget, findModel: findModel, verifyModel: verifyModel })
  ...
  const result = await runReview({ target: reviewTarget, signals: signals, findModel: findModel, verifyModel: verifyModel })
  survivors = result.survivors
  acTable = result.acTable
}
