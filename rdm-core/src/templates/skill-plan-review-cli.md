...
Pass `$ARGUMENTS` through to the `rdm-wf-plan-review` Workflow with any `--rdm-bin <path>` and `--project <name>` pairs removed first — they travel only as the structured `rdmBin`/`project` keys below, because the workflow's tokenizer would read their values as target slugs. It accepts the same four target forms:
