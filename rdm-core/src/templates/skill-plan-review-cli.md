...
Pass `$ARGUMENTS` through to the `rdm-wf-plan-review` Workflow with any `--rdm-bin <path>`, `--project <name>` and `--max-refutations N` pairs removed first — they travel only as the structured `rdmBin`/`project`/`maxRefutations` keys below, because the workflow's tokenizer would read their values as target slugs. It accepts the same four target forms:
...
- **`phases`** — for a `--roadmap` target, the phase stems to sweep:
  ...
  `rdm roadmap show <slug> {proj_flag} --format json`. **Omit it and
  ...
  phase/<roadmap-slug>/<stem> {proj_flag} --format json` — the top-level
...
- **`priorReviews`** — `rdm review list --on <ref> {proj_flag}
...
- **`wontFixedTexts`** — the titles from `rdm search "" --tag
  plan-review --status wont-fix --type task {proj_flag} --format json`. Absent
...
- **`maxRefutations`** — the per-run refutation budget: at most N consolidated
  ...
  max_refutations --raw {proj_flag}` (the same `project` passed below), which
...
- **`rdmBin`** / **`project`** — the rdm executable every reviewer's command
  ...
  subcommands (the supplied `--project`, else the name in `{proj_flag}`).
...
2. **Act on the findings** — apply a small, localized plan fix by writing the whole
   ...
   rdm task create <slug> --title "Plan review finding: description" --body "Details." --tags plan-review --no-plan-review --no-edit {proj_flag}
...
4. **Write the result back** by reading the current full body, splicing in the `## Intent` section, and writing the complete body back — bodies are whole-document-authoritative, there is no patch/diff mechanism:
   ```bash
   rdm roadmap update <slug> --body "<full updated body>" --no-edit {proj_flag}
   # or: rdm task update <slug> --body "<full updated body>" --no-edit {proj_flag}
