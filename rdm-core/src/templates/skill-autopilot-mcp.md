...
Drive **one** rdm roadmap from `not-started` to `reviewed` with no per-phase human approval. This skill drives the loop **itself**, in prose: the two Workflow-tool calls it makes are `estimate` (the difficulty pre-pass, a real parallel fan-out) and `dispatch-phase` (one phase's plan → plan-review → implement → code-review pipeline). Every other step — fetching the next phase, persisting an advance or a park, reading a write back to confirm it landed — is a direct MCP tool call this skill makes itself, because it is already a live agent with these tools in context. See [`docs/workflow-vs-prose-boundary.md`](docs/workflow-vs-prose-boundary.md) for why the drive loop specifically (a low-iteration sequential policy driver) stays prose while `dispatch-phase` and `estimate` stay Workflow scripts (real fan-out over a fixed mechanism).
...
1. **Single roadmap.** The loop never roams to another roadmap — choosing which roadmap to advance stays a human decision. Every `{t_next}` / `{t_phase_update}` / `{t_phase_show}` call below is scoped with the **same fixed** `roadmap: "<slug>"` throughout this run; nothing ever substitutes a different one.
2. **`main` is never touched.** Autopilot leaves every reviewed phase on the `roadmap/<slug>` branch; landing to `main` is the separate **`rdm-land`** skill. There is no `--land` flag here, and no tool call in this loop ever touches `main`.
3. **No `Done:` trailer.** This skill never emits a `Done:` line: its advance step only persists the status the OUTCOME carries, directly via `{t_phase_update}` (a status-only write). **`rdm-land` is the land-time writer** — it reads the OUTCOME's `writesCompletion: true` and synthesizes the trailer from the item's identifiers via `rdm hook done-line`, amending it onto the branch tip before the rebase.
4. **`--permission-mode auto` for unattended runs.** Launch with `--permission-mode auto` (or `bypassPermissions` in a sandbox) so this skill's own tool calls and the `estimate`/`dispatch-phase` Workflow calls don't block on a permission prompt.
...
- Call `{t_phase_list}` with `project: {proj_param}, roadmap: "<slug>"` and take the parsed array verbatim as `phaseList`. It feeds the `estimate` Workflow's unestimated-phase filter directly — do not filter or summarize it yourself.
- Call `{t_next}` with `project: {proj_param}, roadmap: "<slug>"` and take the parsed object verbatim as `next`. This loop consumes this **one-shot, on the first loop iteration only**; every later iteration re-reads live state via the same tool directly, because `{t_next}` is what steps the cursor forward once a phase's status is persisted. Fetch it fresh at invocation time and never cache it across runs.

`mechanicalModel` is **deliberately omitted here**: there is no MCP model-resolve tool, so this shim cannot produce it and must never invent one. The `estimate` Workflow's own `model:mechanical` bootstrap agent remains the path on MCP — a failure there still produces the same fail-closed **`mechanical-model-unresolved`** stop named in step 5's known-good allowlist, even though this shim's own steps never depend on a mechanical model id.

### 3. Run the estimate pre-pass — one Workflow call, always

Invoke the **`estimate` Workflow** via the Workflow tool with `{ roadmap, phaseList }` (omit `phaseList` when you couldn't gather it) to rate every unestimated phase in one parallel fan-out and persist each difficulty (the model tier derives automatically). Run this call **unconditionally**, even if `phaseList` shows zero unestimated phases — it is a cheap no-op fan-out in that case; do not skip it as an optimization.
...
If the `estimate` invocation itself errors or throws, log a warning and continue straight into the drive loop **non-fatally** — an unrated phase simply falls back to whatever tier `{t_next}` reports (empty/medium) when it's dispatched. Do not let a failed pre-pass abort the run.

### 4. Enter the drive loop

Maintain, in this skill's own working context (nothing here is persisted by rdm): `dispatchCount = 0`, `completed = []` (ordered), `escalations = []` (ordered), and — **only when `planOnly` is set** — a `planOnlySeen` set of stems already plan-vetted this run. There is no "seen" tracking in normal mode; normal-mode progress is driven purely by the persisted phase status that an advance/park write changes, which `{t_next}` reads to step forward.
...
2. **Fetch the next phase.** Use the hoisted `next` value from step 2 on the very first iteration only; every later iteration, call `{t_next}` with `project: {proj_param}, roadmap: "<slug>"` and read its result yourself.
3. **Classify the result** (mirrors `interpretNext`):
   - `result: "phase"` with a non-empty `stem` → work it (go to 4).
   - `result: "phase"` **missing** `stem` → this is malformed. Stop with `stopReason: unparseable` — **never** treat a malformed or unrecognized payload as `"nothing"` (that reason is reserved for a genuine, well-formed "no actionable phase" answer). Record a summary-only escalation `{ stem: "(fetch:next)", reason: "[fetch] unparseable rdm-next payload: <bounded description of the raw output>" }` — since no phase stem is known at this point, this entry is never parked via `{t_phase_update}` and will **not** appear in `rdm review blocked`; it only ever appears in this run's printed summary.
...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   - Under `--plan-only`, if `S` is already in `planOnlySeen`, stop with `stopReason: plan-only-exhausted` and go to step 5 — a plan-only pass never advances or persists a terminal status, so `{t_next}` will keep returning the same stem forever; this in-memory check is what detects the repeat.
   ...
   - **Interpret the outcome** (mirrors `interpretOutcome`):
     - `outcome: "reviewed"`, **not** plan-only → **advance**: call `{t_phase_update}` with `project: {proj_param}, roadmap: "<slug>", phase: S, status: <OUTCOME.status || "reviewed">`, then call `{t_phase_show}` with `project: {proj_param}, roadmap: "<slug>", phase: S` and confirm `status` matches. Retry the write+read-back up to **2** times total. On success: append `S` to `completed`, log `"phase S reviewed — advancing"`, continue the loop from step 1. On repeated failure: park `S` (below) with reason `"[code] advance to reviewed failed repeatedly"` — never report a false completion.
   ...
   - **Park**: call `{t_phase_update}` with `project: {proj_param}, roadmap: "<slug>", phase: S, status: "blocked", reason: "<reason>"`, then call `{t_phase_show}` with `project: {proj_param}, roadmap: "<slug>", phase: S` and confirm `status: "blocked"`. Retry up to **2** times total. Whether or not the read-back ever confirms, append `{ stem: S, reason }` to `escalations` and continue the loop from step 1 — an unconfirmed park write must never abort the run before it can print its summary; log a loud warning in that case instead (the plan-repo status may not reflect the park, but the escalation is still recorded here).
...
escalations awaiting review (<n>): <"none", or the block below>
  - <stem> [<stage>]: <reason>
  ...
review the queue: rdm review blocked
...
- `escalations awaiting review` renders each entry as `<stem> [<stage>]: <reason>`, where `<stage>` is parsed from the reason's leading `[tag]` (defaulting to `code` if unparseable), followed by the `rdm review blocked` pointer line — a CLI command for the human reading the summary, since there is no MCP tool for browsing blocked items.
...
- This skill is the **active driver**: every dispatched phase actively runs review (`dispatch-phase`'s code review is the canonical review pipeline `dispatch-phase.js` embeds) before advancing, so nothing is left parked in `needs-review`.
- No `Done:` line is ever written here — this skill's advance step only persists the status the OUTCOME carries, directly via `{t_phase_update}`. **`rdm-land` is the land-time writer**: it reads the OUTCOME's `writesCompletion: true` and synthesizes the trailer from the item's identifiers via `rdm hook done-line`, amending it onto the branch tip before the rebase. No pre-step is required.
