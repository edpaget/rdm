...
Drive **one** rdm roadmap from `not-started` to `reviewed` with no per-phase human approval. This skill drives the loop **itself**, in prose: the one Workflow-tool call it makes is `dispatch-phase` (one phase's plan → plan-review → implement → code-review pipeline). Every other step — fetching the next phase, persisting an advance or a park, reading a write back to confirm it landed — is a direct MCP tool call this skill makes itself, because it is already a live agent with these tools in context. See [`docs/workflow-vs-prose-boundary.md`](docs/workflow-vs-prose-boundary.md) for why the drive loop specifically (a low-iteration sequential policy driver) stays prose while `dispatch-phase` stays a Workflow script (real fan-out over a fixed mechanism).
...
4. **`--permission-mode auto` for unattended runs.** Launch with `--permission-mode auto` (or `bypassPermissions` in a sandbox) so this skill's own tool calls and the `dispatch-phase` Workflow call don't block on a permission prompt.
...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - Otherwise, invoke the **`dispatch-phase` Workflow** via the Workflow tool with exactly `{ roadmap: <slug>, phase: S, planOnly, rdmBin, project: {proj_param} }`, plus `maxPlanRevise` and/or `maxCodeRework` **only when this run's `$ARGUMENTS` set them**. `rdmBin` is the exact rdm executable the workflow's own Bash agents invoke and is REQUIRED even on MCP — the workflow shells out even though this shim does not.
