...
2. Invoke the `rdm-wf-dispatch-phase` Workflow (`.claude/workflows/rdm-wf-dispatch-phase.js`) via the `Workflow` tool with `{ task: <slug>, taskMeta, alreadyInProgress: true, rdmBin, project }`; block for its returned OUTCOME. The task-mode OUTCOME is keyed by `task` (the slug), not `roadmap`/`phase`. `rdmBin` is the exact rdm executable you invoke (e.g. `rdm` on PATH, or a repo-local build path) and is **REQUIRED** — the workflow errors without it rather than falling back to a `PATH`-resolved `rdm`. `project` is optional and is the project name used in `{proj_flag}`, appended only to project-scoped commands.
