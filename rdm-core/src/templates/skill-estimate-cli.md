...
1. **Parse `$ARGUMENTS`** into a config object:
   ...
   - `project` — the value following `--project` when given, otherwise the name in `{proj_flag}`; omit it only when no project name applies, leaving rdm's own `RDM_PROJECT`/`default_project` chain to resolve it. `<proj-flag>` below is ` --project <project>`, or nothing when omitted.
...
3. **Invoke the `rdm-wf-estimate` workflow** via the Workflow tool with `{ roadmap, phase, phaseList, rdmBin, project }` (omit `phase` when not supplied, and `project` when no project name applies). Pass `args` as a JSON object, never a stringified value.
