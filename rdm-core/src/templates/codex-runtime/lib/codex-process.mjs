// Codex subprocess transport shared by the runtime and phase-2 spike fixtures.
...
// Fixed role restrictions, not caller-overridable model configuration. User
// config is ignored below; managed/project MCP configuration is still a host
// concern. These flags are not a universal MCP or filesystem-read allowlist.
// Reference: https://learn.chatgpt.com/docs/config-file/config-reference
const ROLE_CONFIG = ['approval_policy="never"', 'web_search="disabled"',
  ...['apps', 'plugins', 'remote_plugin', 'browser_use', 'computer_use', 'hooks',
    'multi_agent', 'multi_agent_v2', 'workspace_dependencies', 'image_generation']
...
/** Validate the closed JSON Schema subset used by the canonical workflows. */
...
export async function runCodex({bin = 'codex', cwd, prompt, schema, model, effort,
  ...
  const temp = await mkdtemp(join(tmpdir(), 'rdm-codex-process-'));
  ...
  try {
    ...
    const args = ['exec', '--sandbox', 'read-only', '-c', `model_reasoning_effort=${JSON.stringify(effort)}`,
      ...ROLE_CONFIG.flatMap(config => ['-c', config])];
