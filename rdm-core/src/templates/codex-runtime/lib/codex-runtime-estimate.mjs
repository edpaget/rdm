 ...
 * Apply writes only unset difficulty, then commits this run's
...
export async function runEstimate({ ctx, roadmap, apply = false, agent }) {
  ...
  // The canonical pipeline returns writeback descriptions. Execute their typed
  // target/difficulty fields through argv, never their shell command strings.
  async function writeback({ stem, difficulty }) {
      ...
      attemptedWrite = true;
      await ctx.rdm(['phase', 'update', stem, '--difficulty', difficulty, '--expected-estimate-snapshot', current.estimate_snapshot, '--no-edit', ...scope], { mutating: true });
      const after = await show(stem);
      if (after.difficulty !== difficulty || after.body !== current.body || hash(after.tags ?? []) !== hash(current.tags ?? [])) throw new Error(`estimate readback failed: ${stem}`);
      const tier = after.model;
      if (typeof tier !== 'string' || !tier) throw new Error(`missing core tier for ${stem}`);
      intended.set(stem, { body: current.body, difficulty, tags: current.tags ?? [], model: tier });
      return tier;
  }
  ...
  try {
    const phaseList = await ctx.rdm(['phase', 'list', ...scope, '--format', 'json'], { json: true });
    if (!Array.isArray(phaseList) || phaseList.some(p => !p || typeof p.stem !== 'string' || !/^phase-[A-Za-z0-9._-]+$/.test(p.stem)) || new Set(phaseList.map(p => p.stem)).size !== phaseList.length) throw new Error('invalid phase list');
    const summary = await run({ roadmap, project: ctx.identity.project, rdmBin: shellQuote(ctx.identity.rdmBin), phaseList });
    if (failure) throw failure;
    if (apply) for (const estimate of summary.estimated) estimate.tier = await writeback(estimate);
    let planCommit;
    if (apply && proposed.length) {
      ...
      for (const [stem, expected] of intended) {
        ...
        if (!committed || hash({ body: committed.body, difficulty: committed.difficulty, tags: committed.tags ?? [], model: committed.model }) !== hash(expected)) throw new Error(`committed estimate verification failed: ${stem}`);
      }
