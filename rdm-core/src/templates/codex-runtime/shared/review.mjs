...
//! `.claude/skills/{rdm-review,rdm-plan-review}/SKILL.md`. Both targets render
//! the SAME prose for a given mode — the target only decides the `{rdm_bin}`
//! placeholder on example commands, which resolves to `rdm` for `shipped` and
//! `./target/debug/rdm` for `local` (this repo's own hard dev-build rule) — the
//! ONE substitution point for both.
//!
...
export {
  ...
  persistDegradationClause,
  persistDegradedSummary,
