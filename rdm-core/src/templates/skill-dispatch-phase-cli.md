...
3. **Invoke the `rdm-wf-dispatch-phase` workflow** via the Workflow tool with `{ roadmap, phase, phaseMeta, alreadyInProgress, rdmBin, project }` (phase mode) or `{ task, taskMeta, alreadyInProgress, rdmBin, project }` (task mode); pass `args` as a JSON object, never a stringified value. Block for its returned OUTCOME.
   - `rdmBin` — the rdm executable you invoke (e.g. `rdm` on PATH, or a repo-local build path); use the same one the commands above used. **Optional**, defaulting to a plain `rdm` on `PATH` when omitted. Resolve an explicit `--rdm-bin <path>` first, then `$RDM_BIN` if set, then let the default apply — use the first that resolves, and never probe the filesystem to choose between them. An explicitly passed value always wins verbatim, and the literal `"rdm"` requests `PATH` resolution deliberately. If this skill was installed from the `rdm` plugin, the appended "Resolving `rdmBin` (plugin install)" section below is the authoritative statement of that order.
