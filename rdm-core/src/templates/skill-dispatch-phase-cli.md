...
- `<proj-flag>` — ` --project <name>` when `--project` was given, otherwise ` {proj_flag}`, or
...
`subagent_type: rdm-effort-<profile effort>` **and** `model: <profile model>`; the per-call model
...
Invoke the **`rdm-wf-plan-review` Workflow** (`.claude/workflows/rdm-wf-plan-review.js`, provisioned automatically by `rdm agent-config claude --skills`)
...
Invoke the **`rdm-wf-review-refute-fix` Workflow** (`.claude/workflows/rdm-wf-review-refute-fix.js`, provisioned automatically by `rdm agent-config claude --skills`)
via the Workflow tool, passing `args` as a JSON object (never a stringified value):
