...
3. **Invoke the `dispatch-phase` workflow** via the Workflow tool with `{ roadmap, phase, phaseMeta, alreadyInProgress, rdmBin, project }` (phase mode) or `{ task, taskMeta, alreadyInProgress, rdmBin, project }` (task mode); pass `args` as a JSON object, never a stringified value. Block for its returned OUTCOME.
