...
3. **Invoke the `rdm-wf-dispatch-phase` workflow** via the Workflow tool with `{ roadmap, phase, phaseMeta, alreadyInProgress, rdmBin, project }` (phase mode) or `{ task, taskMeta, alreadyInProgress, rdmBin, project }` (task mode); pass `args` as a JSON object, never a stringified value. Block for its returned OUTCOME.
   - `rdmBin` — the exact rdm executable you invoke (e.g. `rdm` on PATH, or a repo-local build path); use the same one the commands above used. **REQUIRED**: the workflow errors without it rather than silently running whichever `rdm` happens to be first on `PATH`. Pass the literal `"rdm"` to opt into `PATH` resolution deliberately.
