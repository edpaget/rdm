...
```bash
rdm phase update <stem-or-number> --roadmap <slug> --no-edit {proj_flag} <<'EOF'
...full revised body...
EOF
rdm task update <slug> --no-edit {proj_flag} <<'EOF'
...full revised body...
EOF
rdm roadmap update <slug> --no-edit {proj_flag} <<'EOF'
...full revised body...
EOF
```
