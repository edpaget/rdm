...
For Claude Code, `--skills --out <dir>` also writes a `.claude/workflows/` directory alongside `.claude/skills/`, containing the Claude Code Workflow-tool scripts (`autopilot.js`, `dispatch-phase.js`, `review-refute-fix.js`) that back the autonomous `rdm-autopilot`/`rdm-dispatch-phase` lane. These currently ship rdm's own dogfood automation as-is — they hardcode `./target/debug/rdm` and `--project rdm` rather than a path/project parameterized for the target repo — so treat them as a starting point to adapt, not a drop-in for an arbitrary project. This is Claude-only (Pi has no Workflow-tool runtime) and only applies with `--out` (not `--user`, since the scripts are project-scoped).
