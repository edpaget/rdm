...
- Session-identity harness: `bash scripts/verify-session-identity.sh` — hermetic multi-process regression for session identity and the changeset journal (`rdm-core/src/session/**`): cross-process stability and inter-session distinctness, rung precedence, stale-lease rejection, journal exactness/disjointness, invisibility to `rdm status` and to a whole-tree commit, measured resolution cost, and (§ I, inverted in phase 5) a grep proving `rdm-store-git/src/commit.rs` carries the changeset commit scope while resolving no session identity of its own. Run it after touching `rdm-core/src/session/**`, `GitStore`'s journal wiring, `FsStore::staged_paths`, or the `rdm session` CLI; design record in `docs/session-identity.md`.
