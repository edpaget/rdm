...
- **In flight:** the autopilot drive loop is migrating to prose (an orchestrating `rdm-autopilot` skill) under `prose-autopilot-orchestration`; `autopilot.js` still runs until its phase 3 retires it. The workflows it drives are unaffected.
