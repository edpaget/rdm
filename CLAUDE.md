...
This installs 11 skills (`rdm:roadmap`, `rdm:dispatch-phase`, etc.) and 5 workflow engines (`rdm:rdm-wf-review-refute-fix`, `rdm:rdm-wf-plan-review`, `rdm:rdm-wf-estimate`, `rdm:rdm-wf-backlog`, `rdm:rdm-wf-document`). The skills invoke bare `rdm` at runtime; the plugin shims resolve the rdm binary via the `--rdm-bin` flag, the `RDM_BIN` environment variable, or PATH lookup.
...
This emits 11 skills and 5 workflow engines to a target directory, with no collision protection or namespace prefixing. Raw emission is **not** the recommended path and carries higher friction — consumers must manage skill naming, binary path resolution, and workflow discovery manually.

The plugin marketplace is recommended for these reasons: automatic namespace prefixing avoids collisions, automatic workflow discovery, and a single installed entity vs. 17 separate file copies.
...
- Finder-collapse harness: `bash scripts/verify-finder-collapse.sh` — hermetic gate for the collapsed-plan-finder A/B instrument (`scripts/lib/finder-collapse.mjs`, `scripts/mine-plan-finder-corpus.mjs`, `scripts/run-finder-collapse.mjs`, `tests/fixtures/finder-collapse/`), which measured whether plan mode's three always-on lenses can run in ONE agent — outcome `no-ship`, pipeline unchanged, figures in `docs/finder-collapse.md` and `docs/token-baseline.json` § `planFinderCollapse`. Run it after touching any of those.
...
The review pipeline is a deterministic **Claude Code Workflow-tool** script under `.claude/workflows/` (a sibling of `.claude/skills/` and `.claude/hooks/`); every driver above it is prose. `rdm agent-config claude --skills --out <dir>` emits both halves: the `rdm-dispatch-phase` skill (`rdm-core/src/templates/skill-dispatch-phase-cli.md`) AND the one `.claude/workflows/*.js` script it and the prose `rdm-autopilot` skill invoke (`rdm-wf-review-refute-fix.js`), embedded via `rdm-core/src/templates/workflows/` and `generate_workflows`/`write_workflows`, byte-identical to this repo's own copy. Neither the autopilot drive loop nor the per-phase unit is a Workflow script: both are prose skills (`rdm-autopilot`, `rdm-dispatch-phase`), and there is no shipped `autopilot.js` or `rdm-wf-dispatch-phase.js` template. The emitted engine is project- and binary-agnostic (`rdmBin`/`project` are runtime args); `lib/*.mjs` and the generator scripts remain unshipped — see `docs/workflow-schemas.md`'s Scope callout.
...
- `.claude/workflows/lib/review.mjs` is the **single canonical review implementation** every surface consumes — find → refute → filter → verdict → gate policy. `buildReviewPipeline(mode)` runs parallel dimension finders → BARRIER → a fresh refuter per GATING finding, bounded by a per-unit **refutation budget** (`maxRefutations`, default 5 from `docs/token-baseline.json` § `determiningFindingRank`; overflow is passed through marked `unrefutedReason: 'budget'`, still floor-filtered, so the bound can only ever add rework, never remove it) — a `suggestion` gates at no tier so it is passed through marked `unrefutedReason: 'non-gating'` and consumes no budget; unknown severities are refuted, fail-safe; a crashed refuter marks its finding `refuterError: true` → drops refuted-or-low-confidence → returns ranked survivors plus the budget accounting, behind `code` (ac/correctness/tests/architecture/api-docs/changelog/security) and `plan` (coherence/architectural-fit/restraint/unit-of-work/intent-alignment) modes, over a CALLER-SUPPLIED reviewer set (`context.reviewers`, resolved by `resolveReviewers`; omitted runs them all, an unrecognised name is dropped silently and shows in `coverage`, and the only refusal is a set that resolves to none — there is no diff-shape inference, no `when` predicate and no signals channel); `classifyOutcome` yields the canonical `reviewed | rework | escalated` vocabulary, and `statusFor`/`writesCompletion` map it onto rdm statuses (escalated → `blocked` for phases *and* tasks). It has TWO `--check`-gated projections: `scripts/gen-workflow-review.sh` stamps its JS block into the workflow consumers, and `scripts/gen-skill-review.sh` renders its `//|` spec prose into the shipped skill templates. That skill projection is **mode-dispatched**: `--mode code` renders `skill-review-cli.md` and `--mode plan` renders `skill-plan-review-cli.md` from the SAME regions, selected by an optional per-line mode tag (`//|` shared, `//|code|`, `//|plan|`); BOTH modes are `--check`-gated by `scripts/verify-workflow-review.sh`. The gate is likewise mode-dispatched policy, not a fork — one `GATE_POLICY[mode][outcome]` table (code: persist an rdm status + permit the completion trailer; plan: clear `needs-plan-review` on `reviewed`, leave it on `rework`/`escalated`, never persist a status), with `STATUS_MAPPING` being exactly `GATE_POLICY.code`. `.claude/workflows/rdm-wf-review-refute-fix.js` has three invocation shapes over the same stamped pipeline: `mode: 'plan'`, and `mode: 'code'` with no item identifier, keep the legacy survivors-only `{ mode, survivors }` shape; `mode: 'code'` with `{ roadmap, phase }` or `{ task }` takes the caller-pinned source identity (`source`/`base`/`expectedHead`/`expectedBranch` — a path, two SHAs, a branch name; each reviewer runs `rdm review source` itself to reach the diff) and composes ONE `classifyOutcome` call over the survivors into the same dispatch-shaped OUTCOME. **It reads nothing and writes nothing** — `persist` and `gate` return ready-to-run Bash (`persistCommands`/`persistScript`, `gateCommands`/`gateScript`) for the ORCHESTRATOR to run, rather than dispatching a writing agent; the persist-ack round trip and its degradation accounting are gone with the agent that produced them. The land-time `Done:` trailer is never written by stamped workflow JS or by `rdm-wf-review-refute-fix.js`'s own optional gate: its format lives in `rdm_core::hook::format_done_directive` (surfaced as `rdm hook done-line`) and is written by the review skill's gate step or by `rdm-land` at land time. `.claude/workflows/rdm-wf-plan-review.js` is the plan-mode standalone consumer: it reuses `buildReviewPipeline('plan')` + `GATE_POLICY.plan` over all four target types (`--task`, `--roadmap`, positional `<slug> [phase]`, `--implementation-plan`), fanning out per-phase with `parallel()` under `--roadmap` and gating each unit independently. **It dispatches finder and refuter agents and nothing else** — no fetch, model, act or gate agents remain. Every read is a `rdm … show --format json` command NAMED in a reviewer's prompt for that reviewer to run itself; everything the caller must supply is an identifier or a short list (`phases` — the stems to sweep, with none the roadmap document is reviewed alone; `tags`; `priorReviews`; `wontFixedTexts`; `reviewers`; `findModel`/`verifyModel`). Every write is RETURNED: `persistCommands`/`persistScript`, `gateAction.commands` (which `filterPlanReviewTag(tags)` builds sibling-preservingly, and which are empty with `tagsUnknown: true` when the caller supplied no tag list), and `roundNote`. `classifyPlanOutcome(survivors)` classifies each unit. `--implementation-plan` grades the plan named by `planSlug` — the one surviving identifier key, since `planText` and the whole plan-body transport negotiation are gone — and skips the gate entirely. Its DRIVER (argument parsing `parsePlanArgs` and the dependency-injected orchestration `runPlanReviewDriver`) is single-sourced in `.claude/workflows/lib/plan-review.mjs` and copied BYTE-IDENTICAL into `rdm-wf-plan-review.js`'s `plan-review-driver` block; `rdm-core/tests/workflow_plan_review_driver.rs` gates the two for byte-equality under `cargo nextest run` and runs `node --test scripts/lib/plan-review-hoist.test.mjs`, which drives the REAL driver over every target kind against a recording fake agent and asserts the dispatched label set contains only finder and refuter labels. Canonical sweep record: [`docs/mechanical-agent-inventory.md`](docs/mechanical-agent-inventory.md).
...
**Update (agent-orchestrated-dispatch phase 6):** the per-phase driver is now **prose**. `.claude/skills/rdm-dispatch-phase/SKILL.md` is the orchestrator — loaded into the main session with `Skill` (an `Agent` subagent has no `Workflow` tool), delegating only the planner and implementer to `Agent`, invoking `rdm-wf-plan-review` and `rdm-wf-review-refute-fix` itself, triaging every comment on the persisted `change/<sha>` review, and writing the terminal `reviewed` status through the core gate (`gates.reviewed` is now ON for rdm's own plan repo). `rdm-do` is a shim onto it and `rdm-autopilot` step 4 enters it with `Skill`; phase 7 retired the `rdm-wf-dispatch-phase` engine, `lib/dispatch-phase.mjs`, its shipped/plugin copies, its emission registration and `scripts/verify-workflow-dispatch.sh` — the record is `docs/workflow-vs-prose-boundary.md` § "Retirement record", and retirement rests on functional acceptance, not measured superiority. The distributed template omits the plan-review call — `rdm-wf-plan-review` ships since phase 26, but a human approve review on the plan is the gate on that surface — and waits on that review instead. It is validated by dogfooding, not by a prose-grep harness: `scripts/verify-skill-dispatch.sh` is deliberately never written, and the two `verify-workflow-do-auto*.sh` harnesses are deleted. Canonical: [`docs/autonomous-loop.md`](docs/autonomous-loop.md) § "The per-phase unit".
...
**Update (unify-code-review phase 6 → phase 7):** phase 6 made review active in every lane that can produce a `needs-review` item — the dispatch code-review stage is the canonical review (stamped from `.claude/workflows/lib/review.mjs` and fed diff-derived `signals`) and returns a `reviewed`/`blocked` status as OUTCOME data rather than persisting it; the prose `rdm-autopilot` skill's own advance/park Bash steps persist that status directly, so nothing is left parked in `needs-review`, and interactive `rdm-do`'s finalize actively invokes `rdm-review` after the human confirm gate before the session stops. With nothing left unreviewed, phase 7 retired the now-redundant `needs-review` auto-review Stop hook / Pi `agent_end` extension outright (`.claude/hooks/rdm-review-on-finalize.sh`, `rdm-core/src/templates/hook-review-on-finalize.sh`, `rdm-core/src/templates/extension-review-on-finalize.ts`, and their `agent-config`/`--hooks` wiring) — see [`docs/autonomous-loop.md`](docs/autonomous-loop.md). The sibling `needs-plan-review` Stop hook / Pi extension has since also been retired (`unify-plan-review` roadmap, phase 4) — see "Plan review" below; `scripts/verify-plan-review-hook-loop.sh`, which tested that hook's own re-prompt loop, was deleted with it.

**Update (dispatch-dev-discipline):** the dispatch runs a phase-time verification gate. The repo-only `dispatch.verify` config key holds ONE project-supplied command (`rdm config set dispatch.verify "<cmd>"`; rejected with `--global`); when it is unset the Stage-0 fetch discovers one from `.github/workflows/`, `docs/principles.md`, then `CLAUDE.md`/`AGENTS.md`, and when nothing resolves the dispatch escalates rather than reporting an unverified pass. The command runs once per implementation attempt and a non-zero exit becomes a blocking finding bounded by the existing `maxCodeRework` budget — no new OUTCOME value. Canonical: [`docs/verify-gate.md`](docs/verify-gate.md).

**Update (no-in-progress-stamp):** the dispatch also stamps the target phase (or task) `in-progress` itself — a best-effort, mechanical write before planning begins; a `--plan-only` run skips it, since it does no implementation. The invoking loop still persists the terminal `reviewed`/`blocked` status itself, since the dispatch writes no terminal status of its own. This is purely an observability fix, not a mutual-exclusion mechanism — see `docs/autonomous-loop.md`.
