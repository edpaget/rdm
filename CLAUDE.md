...
- Editor/plugin-loop contract harness: `bash scripts/verify-plugin-loop.sh` — hermetic end-to-end regression for the argv/JSON contract an editor plugin integration depends on when it shells out to rdm directly: the create/list/update/show/search/info round-trip (including a stdin-supplied body and a fuzzy-typo search match), a portable FIFO-based watchdog proving `create` blocks reading stdin to EOF and unblocks promptly (the one stdin-EOF behavior `rdm-cli/tests/cli_task.rs` doesn't cover), and error-surfacing/stream separability — a bad command's non-zero exit and actionable stderr, and the `needs_review_warning` (triggered via a branched `fixture_code_repo`, not the plan repo, since the warning reads the invoking process's CWD) landing on stderr only, never contaminating `--format json` stdout. The `--body`-authoritative / update-never-reads-stdin / `--body ""`-rejection / `--clear-body` matrix is deliberately NOT reimplemented here — it's owned by `rdm-cli/tests/cli_task.rs` under `cargo nextest run` (`body_flag_beats_stdin`, `task_update_body_flag_beats_stdin`, `task_update_tags_ignores_stdin`, `task_update_status_ignores_stdin`, `task_update_empty_body_refuses_clobber`, `task_update_clear_body_succeeds`). Run it after touching `resolve_body`/`map_body_clobber` or `needs_review_warning` in `rdm-cli/src/commands/`, or any of the exercised task/search/info command surfaces.
