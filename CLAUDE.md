...
- Journal-truncation-race harness: `bash scripts/verify-journal-truncation-race.sh` — hermetic multi-process regression for the changeset journal's truncation-vs-append race: 40 parallel mutations x 6 concurrent commits under one `RDM_SESSION` asserting no authored file is stranded and the journal folds to nothing; a deterministic two-real-process interleave at the `RDM_HARNESS_JOURNAL_BARRIER` seam proving a record appended while a commit is parked survives, including a rewrite of a path that commit is landing; a semantic-empty guard gating the fold, never file absence; and (§ 5) a second deterministic interleave at the `RDM_HARNESS_APPEND_BARRIER` seam running `rdm session gc` — the one non-append operation — from a process sharing neither session id nor lease with a parked appender, proving the append is redone rather than destroyed (no lease can ever name a rung-1/rung-3 changeset, so gc's liveness check is blind by construction; an append that keeps losing escalates to compaction's own lock, which makes the sweep skip). and (§ 6) a third interleave at the `RDM_HARNESS_COMPACT_BARRIER` seam covering the staleness residual — `AdvisoryLock::acquire` takes a lock over on age alone, so a compaction slower than `COMPACT_LOCK_STALE_AFTER` can be dispossessed mid-run; the parked sweep must notice via `AdvisoryLock::still_held` and abandon its rewrite rather than renaming over the record the successor appended under the lock it now holds. All three sharp sections carry mutant-binary self-tests sharing one build (§ 2b restores the read-modify-write `truncate`, § 5b stubs the append's liveness check to report every write as live, § 6b neuters compaction's ownership re-check) asserting the loss reappears. Its § 1 also disentangles the separately-filed pre-flush index-read race. Run it after touching `record`/`truncate`/`append_line`/`read_journal`/`compact`/`gc_changesets` in `rdm-core/src/session/journal.rs`, `AdvisoryLock` in `rdm-core/src/lock.rs`, either `journal::truncate` call site in `rdm-store-git` (`commit_changeset_id`, `commit_whole_tree`), the `rdm session gc` sweep, or any of the three barrier seams.
