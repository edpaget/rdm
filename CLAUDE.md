...
**Hook reconciliation (verified read-only, no code changes):** `scripts/verify-auto-review-hook-loop.sh`, `scripts/verify-plan-review-hook-loop.sh`, and `scripts/verify-worktree-review-loop.sh` all drive the real Stop-hook scripts off directly-set rdm state (status/tags), never off a specific driver — a green run across all three shows the `needs-review` and `needs-plan-review` Stop hooks are agnostic to whatever set that state, workflow or skill. One finding worth recording explicitly: the `needs-review` hook is **dormant by design** under workflow-driven runs — `dispatch-phase`/`autopilot` persist `reviewed`/`blocked` status directly and never stamp `needs-review` (dispatch-phase's code-review stage is the canonical review, stamped from `.claude/workflows/lib/review.mjs` and fed diff-derived `signals`), so the hook never fires in that lane.

**Update (unify-code-review phase 6):** the hook is now dormant in the **interactive** lane too. `rdm-do`'s finalize no longer parks an item in `needs-review` for the hook to pick up — after the human confirm gate it *actively* invokes `rdm-review` (the generated projection of the same canonical source), which drives the item out of `needs-review` to `reviewed` / `in-progress` / `blocked` before the session stops, so the hook reaches its CLEARED path. `needs-review` survives only as a transient marker between the implementation commit and the review. Review is therefore active in every lane, which is the precondition for retiring the hook outright in phase 7.
