...
- `review-refute-fix` is the first shared pipeline: `buildReviewPipeline(mode)` runs parallel dimension finders → a fresh refuter per finding → drops refuted-or-low-confidence → returns ranked survivors, behind `code` (ac/correctness/tests/architecture) and `plan` (coherence/architectural-fit/unit-of-work) modes. `.claude/workflows/review-refute-fix.js` is a thin standalone wrapper.
- Review pipeline harness: `bash scripts/verify-workflow-review.sh` — hermetic DRIFT / HYGIENE / BEHAVIOR regression. It runs `gen-workflow-review.sh --check`, greps the workflow scripts for forbidden `Date.now(`/`Math.random(`, and drives the shared module in Node (stdlib-only, no packages; `node` pinned in `.mise.toml`) with an injected fake agent + reference `pipeline`/`parallel`, asserting a planted refutable finding is dropped, a real one survives, the confidence floor drops low-confidence findings, a fresh refuter grades each finding, context is threaded into every prompt, and output is deterministic across both modes. Run it after touching `.claude/workflows/lib/review-refute-fix.mjs`, the generator, or the workflow consumers.
...
**Hook reconciliation (verified read-only, no code changes):** `scripts/verify-auto-review-hook-loop.sh`, `scripts/verify-plan-review-hook-loop.sh`, and `scripts/verify-worktree-review-loop.sh` all drive the real Stop-hook scripts off directly-set rdm state (status/tags), never off a specific driver — a green run across all three shows the `needs-review` and `needs-plan-review` Stop hooks are agnostic to whatever set that state, workflow or skill. One finding worth recording explicitly: the `needs-review` hook is **dormant by design** under workflow-driven runs — `dispatch-phase`/`autopilot` persist `reviewed`/`blocked` status directly and never stamp `needs-review` (dispatch-phase inlines its own code-review stage as part of the pipeline), so the hook only fires for interactive `rdm-do`, which does stamp `needs-review` on finalize. This is intended architecture, not a gap, and is not tracked as a follow-up task.
