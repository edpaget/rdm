...
**Update (dispatch-dev-discipline, project directives):** a project's own prose rules (`.claude/rules/`, `AGENTS.md`, `.cursor/rules/`, `.clinerules`, `.windsurf/rules/`, `.github/copilot-instructions.md` — never `CLAUDE.md`, already loaded per subagent; or the repo-only `dispatch.directives` key, which REPLACES that discovery) are resolved read-only by `rdm dispatch directives --format json` and injected VERBATIM into the dispatched implementer's and every reviewer's prompt. Guidance, never enforcement: no finding, no gate, no status. Bounded at 8000 bytes/source and 16000 total, single-sourced in Rust, with over-bound sources SKIPPED WHOLE (never truncated) and reported in three channels. Canonical: [`docs/project-directives.md`](docs/project-directives.md). Harness: `bash scripts/verify-project-directives.sh` — run it after touching `rdm-core/src/directives.rs`, `rdm-cli/src/commands/dispatch.rs`, the `directives` region of `.claude/workflows/lib/dispatch-phase.mjs`, or `DIRECTIVES_PREAMBLE` in `.claude/workflows/lib/review.mjs`.
