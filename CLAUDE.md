...
- The autopilot drive loop has migrated to prose (an orchestrating `rdm-autopilot` skill, `.claude/skills/rdm-autopilot/SKILL.md`) under the `prose-autopilot-orchestration` roadmap; phase 3 retired `autopilot.js`/`lib/autopilot.mjs` and its harness `scripts/verify-workflow-autopilot.sh`.
