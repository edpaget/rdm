...
#[test]
fn conditional_estimate_accepts_append_body_and_still_enforces_the_precondition() {
    // The one estimate write carrying an optimistic-concurrency precondition used
    // to be the only one that could NOT append, so its caller had to read the
    // whole body out and hand it back — the clobber-shaped write `--append-body`
    // exists to remove. The snapshot is checked against the STORED body before
    // the update applies, so the two compose safely.
    let dir = TempDir::new().unwrap();
    init_with_roadmap(&dir);
    create_phase(&dir, "core", "Core");
    rdm()
    ...
    let phase = dir
        .path()
        .join("projects/fbm/roadmaps/two-way/phase-1-core.md");
    let content = fs::read_to_string(&phase).unwrap();
    assert!(
        content.contains("Original body\n\n## Estimate\n\neasy — small"),
        "the guarded write appended without touching the prior body, got: {content}"
    );

    // The precondition is still live: the now-stale snapshot is refused, and the
