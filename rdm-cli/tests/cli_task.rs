...
#[test]
fn task_update_append_body_keeps_the_existing_body() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
    create_task(&dir, "fix-bug", "Fix the bug");
    ...
    let content = fs::read_to_string(dir.path().join("projects/fbm/tasks/fix-bug.md")).unwrap();
    assert!(
        content.contains("existing content\n\n## Note\n\nappended"),
...
#[test]
fn task_update_append_body_conflicts_with_body_and_clear_body() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
    create_task(&dir, "fix-bug", "Fix the bug");
