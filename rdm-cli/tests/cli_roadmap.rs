...
#[test]
fn roadmap_update_clear_body_succeeds() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
...
#[test]
fn roadmap_update_empty_body_ok_when_already_empty() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
...
#[test]
fn roadmap_update_clear_body_conflicts_with_body_flag() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
...
#[test]
fn roadmap_update_title_renames_in_place() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
    rdm()
        .arg("--root")
    ...
    rdm()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Updated 'two-way'"));

    // The new title is reflected via `show`; the slug is unchanged.
    rdm()
        .arg("--root")
        .arg(dir.path())
        .args(["roadmap", "show", "two-way", "--project", "fbm"])
    ...
    // INDEX.md was regenerated by the update without error. The index renders
    // roadmaps by slug (not title), so we assert the slug is present rather than
    // the title.
...
#[test]
fn roadmap_update_empty_title_rejected() {
    let dir = TempDir::new().unwrap();
    init_with_project(&dir);
    rdm()
        .arg("--root")
    ...
    // Title is untouched after the rejected update.
    rdm()
        .arg("--root")
        .arg(dir.path())
        .args(["roadmap", "show", "two-way", "--project", "fbm"])
