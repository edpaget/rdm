...
/// Writes the Claude Stop-hook script (executable) and merges the hook
/// registration into `settings.json`.
///
/// # Errors
///
/// Returns an error if the output directory cannot be resolved, a file
/// write/permission change fails, or the settings merge fails.
fn write_claude_hook(platform: Platform, user: bool, out: &Option<PathBuf>) -> Result<()> {
    ...
    let merged = agent_config::merge_stop_hook_into_settings(existing.as_deref())
        .with_context(|| format!("failed to merge Stop hook into {}", settings_path.display()))?;
    write_output(&settings_path, merged.as_bytes())
}

/// Writes the Pi `rdm-review.ts` extension file.
///
/// # Errors
///
/// Returns an error if the output directory cannot be resolved or the file
/// write fails.
