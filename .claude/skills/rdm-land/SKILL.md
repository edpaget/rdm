...
6. **Mark every item in the target set `done`, directly.** Read the landed tip: `git -C <primary> rev-parse main`. For a `task/<slug>` land, that tip *is* the task's own last commit — the fast-forward advanced by exactly its un-landed work. For a roadmap-scoped land — bare `<roadmap>` **or** `<roadmap>/<phase>` — record the landed tip, not each phase's own last commit, for every phase in the target set: the roadmap branch is shared, and attributing each phase's own distinct commit is not sound in general (a change-review head can predate the commit that actually finalized it — e.g. under a stale-review `--override-gate` waiver — and `started_head` isn't reliably populated for every phase). The landed tip gives up per-phase precision but can never cite a wrong, unlanded, or nonexistent commit. For each item in the target set:
   ```bash
