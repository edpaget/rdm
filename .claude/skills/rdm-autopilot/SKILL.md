...
2. **Estimate if needed:** if the phase's `difficulty` is unset in the `./target/debug/rdm next` output, run the `rdm-estimate` skill on it first (`<slug> <phase>`) so it has a difficulty and a derived model tier before dispatch. If it already has a difficulty, skip estimation.
