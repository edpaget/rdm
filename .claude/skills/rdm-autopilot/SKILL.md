...
- Run `<rdmBin> phase list --roadmap <slug><proj-flag> --format json` and take the parsed array verbatim as `phaseList`. It feeds the `estimate` Workflow's unestimated-phase filter directly (mirroring `rdm-estimate`'s own contract) — do not filter or summarize it yourself.

### 3. Run the estimate pre-pass — one Workflow call, always

Invoke the **`estimate` Workflow** via the Workflow tool with `{ roadmap, mechanicalModel, phaseList, rdmBin, project }` (omit `phase` — autopilot always estimates the whole roadmap, never a single phase number). `rdmBin` and `project` are the same resolved values from step 1 — `estimate.js`'s own `parseEstimateArgs` validates them via `resolveRdmBin`/`parseProjectArg`, so passing them here is what lets `estimate` run against this loop's caller-supplied binary and project instead of an ambient default. Run this call **unconditionally**, even if `phaseList` shows zero unestimated phases — it is a cheap no-op fan-out in that case, the same always-invoke-and-let-it-no-op design the `rdm-estimate` skill itself uses; do not skip it as an optimization.
...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - Otherwise, invoke the **`dispatch-phase` Workflow** via the Workflow tool with exactly `{ roadmap: <slug>, phase: S, planOnly, rdmBin, project }`, plus `maxPlanRevise` and/or `maxCodeRework` **only when this run's `$ARGUMENTS` set them** — this is the exact payload shape `dispatch-phase` is invoked with today, preserved byte-for-shape apart from the two required environment args `rdmBin`/`project` added by `parameterize-dispatch-phase-and-contract`; `rdmBin`/`project` are the same resolved values from step 1, passed as bare keys (never re-literalized here). `rdmBin` is REQUIRED and the workflow errors without it.
