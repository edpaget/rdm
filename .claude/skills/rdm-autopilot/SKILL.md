...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - Otherwise, invoke the **`dispatch-phase` Workflow** via the Workflow tool with exactly `{ roadmap: <slug>, phase: S, planOnly, rdmBin: "./target/debug/rdm", project: "rdm" }`, plus `maxPlanRevise` and/or `maxCodeRework` **only when this run's `$ARGUMENTS` set them** — this is the exact payload shape `dispatch-phase` is invoked with today, preserved byte-for-shape apart from the two required environment args `rdmBin`/`project` added by `parameterize-dispatch-phase-and-contract`; `rdmBin` is REQUIRED and the workflow errors without it.
