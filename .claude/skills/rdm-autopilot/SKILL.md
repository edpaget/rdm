...
**Input** (`$ARGUMENTS`): a **required roadmap slug**, a **required `--rdm-bin <path>`**, optionally followed by `--project <name>`, `--max-phases N`, `--plan-only`, `--max-plan-revise N`, and/or `--max-code-rework N`. The slug names the single roadmap this run drives. If no slug is given, stop before invoking anything and say so — do not attempt a partial estimate or drive-loop start. If `--rdm-bin` is missing, stop before invoking anything and say so, the same pre-flight way — this loop never falls back to a silently PATH-resolved `rdm` (pass the literal sentinel `rdm` to opt into that explicitly, mirroring `rdm-wf-dispatch-phase`'s own `resolveRdmBin` contract).

Every Bash command and Workflow payload below is written against two placeholders resolved once in step 1: `<rdmBin>` — the resolved executable from `--rdm-bin` — and `<proj-flag>` — ` --project <project>` when `--project` was given, or nothing at all when it was omitted (never an empty `--project` value).
...
- `rdmBin` — the required executable path following `--rdm-bin`. Missing → stop immediately, before step 2, and say so (same pre-flight-stop pattern as a missing roadmap slug). Accepts the literal sentinel `rdm` to opt into `PATH` resolution. This resolves the `<rdmBin>` placeholder used everywhere below.
...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - Otherwise, invoke the **`rdm-wf-dispatch-phase` Workflow** via the Workflow tool with exactly `{ roadmap: <slug>, phase: S, planOnly, rdmBin, project }`, plus `maxPlanRevise` and/or `maxCodeRework` **only when this run's `$ARGUMENTS` set them** — this is the exact payload shape `rdm-wf-dispatch-phase` is invoked with today, preserved byte-for-shape apart from the two required environment args `rdmBin`/`project` added by `parameterize-dispatch-phase-and-contract`; `rdmBin`/`project` are the same resolved values from step 1, passed as bare keys (never re-literalized here). `rdmBin` is REQUIRED and the workflow errors without it.
