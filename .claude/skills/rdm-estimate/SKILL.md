...
4. **Run each returned writeback, then print the summary.** The workflow persists nothing: every rated phase comes back with `writebackCommands` / `writebackScript` — read the phase body, write it back with the `## Estimate` note and `--difficulty`, then read the phase back. Run each one in Bash, in order, and report the exit status; the last command in each is what shows the core-derived model tier. Then print the returned summary verbatim — it lists each phase rated this run with its difficulty and one-line justification, plus the phases skipped because they were already estimated.
