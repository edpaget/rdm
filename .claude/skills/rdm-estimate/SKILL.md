...
1. **Parse `$ARGUMENTS`** into a config object:
   ...
   - `project` — the value following `--project` when given, otherwise `rdm` (this repo's project); omit it only when no project name applies, leaving rdm's own `RDM_PROJECT`/`default_project` chain to resolve it. `<proj-flag>` below is ` --project <project>`, or nothing when omitted.
