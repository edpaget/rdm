...
The workflow rates only phases whose **difficulty is unset** and **skips** any phase that already has a difficulty (idempotent — re-running never re-rates or overwrites a human-set or previously-estimated value). The model **tier derives in rdm-core** from the difficulty (`Difficulty::model_tier`); the returned writeback commands set `--difficulty` only, never `--model`, and end by reading the phase back so the resulting tier is visible.
...
4. **Run each returned writeback, then print the summary.** The workflow persists nothing: every rated phase comes back with `writebackCommands` / `writebackScript` — capture the `## Estimate` note in a quoted heredoc, set `--difficulty` and add the note with `--append-body`, then read the phase back. Run each one in Bash, in order, **exactly as returned**: there is nothing to substitute, and the phase body is never read, carried, or rewritten by you. Report the exit status; the last command in each is what shows the core-derived model tier. Then print the returned summary verbatim — it lists each phase rated this run with its difficulty and one-line justification, plus the phases skipped because they were already estimated.
