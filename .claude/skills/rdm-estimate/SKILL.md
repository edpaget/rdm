...
2. **Invoke the `estimate` workflow** via the Workflow tool with `{ roadmap, phase }` (omit `phase` when not supplied). Pass `args` as a JSON object, never a stringified value.
3. **Print the returned summary verbatim.** It lists each phase estimated this run with its assigned difficulty, the core-derived model tier, and the one-line justification, plus the phases skipped because they were already estimated.
