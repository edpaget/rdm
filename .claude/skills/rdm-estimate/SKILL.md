...
2. **List the phases**: `./target/debug/rdm phase list --roadmap <slug> --project rdm`. The output shows each phase's number, stem, status, and current difficulty.
...
4. **For each selected phase**, in order:
   1. Read the body: `./target/debug/rdm phase show <stem-or-number> --roadmap <slug> --project rdm`.
...
- To force a re-estimate of a phase, a human first clears its difficulty (`./target/debug/rdm phase update <phase> --clear-difficulty --no-edit --roadmap <slug> --project rdm`); the next run will then pick it up.
