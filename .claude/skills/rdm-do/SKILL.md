...
3. **Enter the orchestrator in this same session** — with the `Skill` tool, never with `Agent`:
   ...
   calls could not be made there (`docs/workflow-schemas.md` § "Orchestrator /
   Workflow-reachability spike"). The orchestrator owns everything from that point: the worktree,
...
`rdm search "" --tag <candidate> --project rdm`). When you are inside a roadmap's worktree and the task
