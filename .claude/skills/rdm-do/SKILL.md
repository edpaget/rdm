...
**Never hand-type the completion trailer.** Its format lives in
`rdm_core::hook::format_done_directive`, surfaced as `rdm hook done-line`. The reviewed work is left
on the `roadmap/<slug>` branch for `rdm-land`; the merge hook flips `reviewed` → `done`.
