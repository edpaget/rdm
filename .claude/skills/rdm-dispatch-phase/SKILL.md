...
2. **Read the phase and its model tier:** `./target/debug/rdm phase show <phase> --roadmap <slug> --project rdm`. Capture the full body (context, steps, acceptance criteria) and the assigned **Model** tier (also visible via `./target/debug/rdm phase list --roadmap <slug> --project rdm`). The body captured here is the **only** phase context you pass downstream — do not pass roadmap-wide or conversation context.
3. **Create/locate the roadmap worktree and mark in-progress:**
   - `./target/debug/rdm worktree add <slug> --project rdm` — pass the **roadmap** ref (not `<slug>/<phase-stem>`); idempotent and prints the worktree path. The first phase creates the `roadmap/<slug>` worktree; every later phase of the same roadmap reuses it. `cd` into the printed path and do all implementation there (run `cargo build` in the worktree and use **that worktree's** `./target/debug/rdm`). The new worktree's branch is based on whatever branch the invoking checkout is currently on at creation time — not always `main` — so a run launched from a feature branch builds on that branch's work. Pass `--base <ref>` only to override (e.g. to force off `main`).
...
7. **Code review:** run the `rdm-review` skill on the result (`<slug> <phase>`). It owns the `needs-review` → `reviewed` gate and the `Done:` line, and it speaks the **same** outcome vocabulary this skill returns — `reviewed | rework | escalated`. Map its outcome onto yours one-for-one:
   ...
   - **escalated (decision/blocker, not a defect)** → if review surfaces a genuine AC ambiguity or architectural decision rather than a fixable defect — or the single rework pass is exhausted without converging — park it as a `code`-stage escalation instead of retrying, and return **escalated**:
     ```bash
