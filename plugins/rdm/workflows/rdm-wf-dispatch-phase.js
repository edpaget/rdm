...
function itemOutcome(fields) {
  ...
  if (isTask) {
    return buildTaskOutcome({
      ...
      coverageRounds: f.coverageRounds,
      planCoverage: f.planCoverage,
...
if (planOnly) {
  ...
  // The plan gate's own DIMENSION-PARTICIPATION accounting rides along for the
  // same reason, so a plan-only run whose plan review lost a dimension is not
  // reported as a complete one. Empty clause on a complete run.
  const planOnlyCoverage = buildReviewCoverage([], planGate.coverageRounds)
  const planOnlySummary =
    'plan-only: plan gate passed' + budgetSummaryClause(planOnlyBudget) + coverageSummaryClause(planOnlyCoverage)
  const o = isTask
    ? {
        task: taskSlug,
        outcome: 'reviewed',
        ...
        reviewCoverage: planOnlyCoverage,
        findings: planFindings,
      }
    : {
        roadmap: roadmap,
