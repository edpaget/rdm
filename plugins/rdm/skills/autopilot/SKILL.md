...
4. **Work the phase**, stem `S`, tier `T` (`next.model`, defaulting to `medium` when unset):
   ...
   - Otherwise, invoke the **`rdm:rdm-wf-dispatch-phase` Workflow** via the Workflow tool with exactly `{ roadmap: <slug>, phase: S, planOnly, rdmBin, project }`, plus `maxPlanRevise` and/or `maxCodeRework` **only when this run's `$ARGUMENTS` set them**. `rdmBin` is the exact rdm executable you invoke (the same one the commands in this loop use) and is REQUIRED — the workflow errors without it rather than falling back to a `PATH`-resolved `rdm`; `project` is the project name used in `--project <PROJECT>`.
