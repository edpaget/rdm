...
Two `Workflow` runs remain in this lane: step 3's `rdm:rdm-wf-estimate` pre-pass and the code review the orchestrator invokes. If either crashes mid-flight, relaunch that same script with an added `resumeFromRunId: '<prior runId>'` argument instead of invoking it fresh; any `agent()` call whose `(prompt, opts)` are byte-unchanged replays its cached result. Four caveats apply every time:
