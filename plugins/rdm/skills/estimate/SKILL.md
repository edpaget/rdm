...
1. **Parse `$ARGUMENTS`** into a config object:
   ...
   - `project` — the value following `--project` (the name in `--project <PROJECT>`); omit it when not supplied, and rdm's own `RDM_PROJECT`/`default_project` chain applies. `<proj-flag>` below is ` --project <project>`, or nothing when omitted.
...
3. **Invoke the `rdm:rdm-wf-estimate` workflow** via the Workflow tool with `{ roadmap, phase, phaseList, rdmBin, project }` (omit `phase` and `project` when not supplied). Pass `args` as a JSON object, never a stringified value.
