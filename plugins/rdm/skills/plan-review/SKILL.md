...
Review the *plan* of an rdm roadmap, phase, or task — not its implementation. `$ARGUMENTS` should be `<roadmap-slug> [phase-number]` for a phase, `--task <slug>` for a task, `--roadmap <slug>` for a whole roadmap, or `--implementation-plan` for reviewing an in-progress `do` implementation plan directly.
...
The specification of that pipeline — which dimensions run, how findings are graded, and what each outcome means — is **generated from the canonical review source** and is shared with `review`, which reviews the diff after implementation instead of the plan before it. It appears under "Review specification" below. The steps here wire it to the CLI.
...
Large findings are filed as tasks instead. Always add `--no-plan-review`: a task filed *from* a plan-review finding must not itself be stamped `needs-plan-review`, or the gate's own output becomes new input to the gate.
```bash
