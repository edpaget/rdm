...
Review the *plan* of an rdm roadmap, phase, or task — not its implementation. This skill is a thin shim over the **`rdm:rdm-wf-plan-review` Workflow** (`rdm:rdm-wf-plan-review`, installed by the `rdm` plugin), which runs the whole pipeline end to end. Invoke that workflow and report its result; the domain notes below exist so a human reader understands what it does and can drive it interactively when the workflow is unavailable.
...
- `--implementation-plan` — review an `do` plan document handed over in context, ahead of implementation. There is **no persisted rdm item** behind this mode, so it is report-only (see the carve-out below).
...
- **`phases`** — for a `--roadmap` target, the phase stems to sweep:
  ...
  `rdm roadmap show <slug> --project <PROJECT> --format json`. **Omit it and
  ...
  entry's own `priorReviews` comes from `rdm review list --on
  phase/<roadmap-slug>/<stem> --project <PROJECT> --format json` — the top-level
...
- **`priorReviews`** — `rdm review list --on <ref> --project <PROJECT>
...
- **`wontFixedTexts`** — the titles from `rdm search "" --tag
...
2. **Act on the findings** — apply a small, localized plan fix by writing the whole
   ...
   rdm task create <slug> --title "Plan review finding: description" --body "Details." --tags plan-review --no-plan-review --no-edit --project <PROJECT>
   rdm commit -m "chore(plan): address plan review findings on <target>"
   ```
...
The gate returns its commands rather than writing them, unconditionally, so every tag write happens in your session where a refusal can be surfaced and reported.
...
4. **Write the result back** by reading the current full body, splicing in the `## Intent` section, and writing the complete body back — bodies are whole-document-authoritative, there is no patch/diff mechanism:
   ```bash
   rdm roadmap update <slug> --body "<full updated body>" --no-edit --project <PROJECT>
   # or: rdm task update <slug> --body "<full updated body>" --no-edit --project <PROJECT>
   rdm commit -m "chore(plan): capture intent on <target>"
   ```

## What the workflow does (domain intent)

The pipeline runs `find → refute → filter → verdict → act → gate`; no finding of a gating severity is surfaced, fixed, or acted on until a *separate* refuter agent has failed to refute it (a non-gating `suggestion` passes through marked `unrefuted: true`). Its full specification — which dimensions run, how findings are graded, what each outcome means — is **generated from the canonical review source** (shared with `review`, which reviews the diff after implementation) and appears under "Review specification" below.
...
`implementation-plan` — an `do` plan document handed over in context
