...
Act on submitted document reviews with verdict `request-changes`: work through each comment, apply the requested edits through `rdm` update commands, and drive the review to `addressed`. This is not `review` (which reviews *implementations*) — this skill acts on *document* reviews of roadmaps, phases, and tasks.
...
```bash
rdm review requests --format json --project <PROJECT>
```
...
```bash
rdm review show <review-id> --format json --project <PROJECT>
```
...
rdm phase update <stem-or-number> --roadmap <slug> --body "$body" --no-edit --project <PROJECT>
# or: rdm task update <slug> --body "$body" --no-edit --project <PROJECT>
# or: rdm roadmap update <slug> --body "$body" --no-edit --project <PROJECT>
```
...
```bash
rdm review update <review-id> --comment <n> --status addressed --applied-commit <sha> --reply "What changed, and whether the anchor resolved (note drift if any)." --project <PROJECT>
```
...
```bash
rdm review update <review-id> --comment <n> --reply "The quoted text changed since the review — did you mean X or Y?" --project <PROJECT>
```
...
```bash
rdm review update <review-id> --comment <n> --status wont-fix --reply "Reasoning for not making this change." --project <PROJECT>
```
...
```bash
rdm review update <review-id> --state addressed --project <PROJECT>
```
