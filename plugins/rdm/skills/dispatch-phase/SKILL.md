...
and is counted into the printed `anchorsDegraded=` line, so there is nothing for you to re-run by hand
for that case. If `review start` itself is refused, park rather than choosing another target.
`gate: false` keeps the status write here, in step 14, where a refusal can be surfaced.
