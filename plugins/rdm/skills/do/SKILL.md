...
4. **Return the orchestrator's OUTCOME verbatim** as your final message, including its `planId` and
   ...
   orchestrator already performed the gated status write, and `land` is the only writer of the
...
reviewed work is left on the `roadmap/<slug>` branch for `land`; the merge hook flips `reviewed` →
`done`.
