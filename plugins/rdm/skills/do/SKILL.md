...
2. Invoke the `rdm:rdm-wf-dispatch-phase` Workflow (`rdm:rdm-wf-dispatch-phase`, installed by the `rdm` plugin) via the `Workflow` tool with `{ roadmap: <slug>, phase: <stem>, phaseMeta, alreadyInProgress: true, rdmBin, project }`; block for its returned OUTCOME. `rdmBin` is the rdm executable you invoke (e.g. `rdm` on PATH, or a repo-local build path) and is **optional**, defaulting to a plain `rdm` on `PATH` when omitted — resolve an explicit `--rdm-bin <path>` first, then `$RDM_BIN` if set, then let the default apply, using the first that resolves and never probing the filesystem to choose. An explicitly passed value always wins verbatim. `project` is optional and is the project name used in `--project <PROJECT>`, appended only to project-scoped commands.
...
2. Invoke the `rdm:rdm-wf-dispatch-phase` Workflow (`rdm:rdm-wf-dispatch-phase`) via the `Workflow` tool with `{ task: <slug>, taskMeta, alreadyInProgress: true, rdmBin, project }`; block for its returned OUTCOME. The task-mode OUTCOME is keyed by `task` (the slug), not `roadmap`/`phase`. `rdmBin` is the rdm executable you invoke (e.g. `rdm` on PATH, or a repo-local build path) and is **optional**, defaulting to a plain `rdm` on `PATH` when omitted — resolve an explicit `--rdm-bin <path>` first, then `$RDM_BIN` if set, then let the default apply, using the first that resolves and never probing the filesystem to choose. An explicitly passed value always wins verbatim. `project` is optional and is the project name used in `--project <PROJECT>`, appended only to project-scoped commands.
