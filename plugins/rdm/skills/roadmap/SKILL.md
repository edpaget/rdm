...
1. **Explore the codebase** to understand the current state relevant to `$ARGUMENTS`. Read key files, search for related code, and build context. Also run `rdm search <topic> --project <PROJECT>` for related existing plan items — an overlapping roadmap, phase, or task you should link to or build on rather than duplicate.
