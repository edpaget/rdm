...
Landing is the one step that **writes to `main`**. It runs **only on explicit invocation** — when you run `/land`, or when `autopilot` is given its opt-in `--land` flag. It **never auto-lands**: nothing in the normal review flow reaches `main` on its own. The fast-forward is exactly what triggers `rdm hook post-commit` on the default branch to mark the item `done`, so the `Done:` line the branch already carries keeps working unchanged.
...
6. **Confirm the item flipped `reviewed → done`.** The fast-forward onto the default branch fires `rdm hook post-commit`, which reads the `Done:` line and marks the item done. Verify with `rdm phase show <phase> --roadmap <slug> --project <PROJECT>`. If the hook did not run (e.g. hooks not installed), apply the idempotent fallback:
   ```bash
   rdm phase update <phase> --status done --commit <sha> --no-edit --roadmap <slug> --project <PROJECT>
   # or, for a task:
   rdm task update <slug> --status done --commit <sha> --no-edit --project <PROJECT>
   ```
