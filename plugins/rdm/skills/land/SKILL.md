...
1. **Read item status** and verify it is `reviewed` (precondition 1). Determine its branch: `roadmap/<slug>`, `task/<slug>`, or the phase's branch. For a bare `<roadmap>` land, also determine the **target set** — every phase that is currently `reviewed`: `rdm phase list --roadmap <slug> --format json --project <PROJECT>` and keep every element with `"status": "reviewed"`. For a `<roadmap>/<phase>` or `task/<slug>` land, the target set is just that one item.
