...
Landing is the one step that **writes to `main`**. It runs **only on explicit invocation** — when you run `/land`, or when `autopilot` is given its opt-in `--land` flag. It **never auto-lands**: nothing in the normal review flow reaches `main` on its own. After the fast-forward, this skill marks the landed item(s) `done` itself — no trailer, no hook dependency.
...
1. **The item is `reviewed`.** Read it: `rdm phase show <phase> --roadmap <slug> --project <PROJECT>` (or `rdm task show <slug> --project <PROJECT>`). If it is not `reviewed` — e.g. still `needs-review`, `blocked`, or already `done` — stop: only reviewed work lands.
...
1. **Read item status** and verify it is `reviewed` (precondition 1). Determine its branch: `roadmap/<slug>`, `task/<slug>`, or the phase's branch. For a bare `<roadmap>` land **or** a `<roadmap>/<phase>` land, also determine the **target set** — every phase on that roadmap that is currently `reviewed`, the named phase included: `rdm phase list --roadmap <slug> --format json --project <PROJECT>` and keep every element with `"status": "reviewed"`. The roadmap branch is shared, so a `<roadmap>/<phase>` land's fast-forward (step 5) can carry other reviewed phases' commits past the one you named — treat it exactly like a bare-roadmap land for completion purposes (step 6). For a `task/<slug>` land, the target set is just that one task — a task's branch is not shared.
...
6. **Mark every item in the target set `done`, directly.** Read the landed tip: `git -C <primary> rev-parse main`. For a `task/<slug>` land, that tip *is* the task's own last commit — the fast-forward advanced by exactly its un-landed work. For a roadmap-scoped land — bare `<roadmap>` **or** `<roadmap>/<phase>` — record the landed tip, not each phase's own last commit, for every phase in the target set: the roadmap branch is shared, so the fast-forward can carry other reviewed phases' commits past the one you named, and attributing each phase's own distinct commit is not sound in general anyway — a phase's approving change-review head can be wrong when the phase was finalized by a commit made *after* that review, and a phase-start record is not reliably populated for every phase. Recording the landed tip for every phase gives up per-phase precision but can never cite a wrong, unlanded, or nonexistent commit. For each item in the target set:
   ```bash
   rdm phase update <phase> --status done --commit <tip-sha> --no-edit --roadmap <slug> --project <PROJECT>
   # or, for a task:
